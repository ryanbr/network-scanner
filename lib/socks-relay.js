...
async function ensureRelay(upstream, forceDebug = false) {
  ...
  // try/finally inside the IIFE so a rejected init still clears the
  // pending entry — otherwise the Map would retain a rejected-Promise
  // entry forever and any subsequent caller for the same key would await
  // that rejection. Same pattern as pending*Lookups in lib/nettools.js.
