  ...
  const onData = async (chunk) => {
    ...
    try {
      ...
      if (phase === 'request') {
        ...
        // Probe a silently-dead upstream (NAT timeout, mobile-tower drop,
        // proxy crash without FIN/RST) within ~60s instead of waiting for
        // the Linux default of ~2 hours. Localhost-side keep-alive on the
        // client socket is omitted -- same kernel, OS surfaces the death
        // immediately via socket close.
 ...
 * { key, port, activeConnections } — the upstream `key` has its trailing
 * `:username` segment stripped using the same regex as closeAllRelays'
 * display path (IPv6-safe). Useful for answering "is my proxy slow
 * because the upstream is saturated, or because the scan is opening
 * too many parallel tunnels?" without enabling forceDebug.
 */
