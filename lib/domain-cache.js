...
class DomainCache {
  ...
  checkAndMark(domain) {
    ...
    // Prevent cache from growing too large
    if (this.cache.size >= this.options.maxCacheSize) {
      this.clearOldestEntries(Math.floor(this.options.maxCacheSize * 0.1));
    }
    ...
    if (this.options.enableLogging) {
      console.log(formatLogMessage('debug', `${this.options.logPrefix} Cache MISS: ${domain} (processing and marked, cache size: ${this.cache.size})`));
    }
