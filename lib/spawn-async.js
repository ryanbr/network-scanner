...
function runProcess(cmd, args, opts = {}) {
  ...
  return new Promise((resolve) => {
    let child;
    try {
      // Defense in depth: never let this fall back to a shell, and reject
      // any cmd containing shell metacharacters even though callers today
      // always pass hardcoded executable names (e.g. 'curl', 'grep').
      if (typeof cmd !== 'string' || !/^[\w./-]+$/.test(cmd)) {
        throw new Error(`refusing to spawn suspicious command: ${cmd}`);
      }
