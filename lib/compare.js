...
function loadComparisonRules(compareFilePath, forceDebug = false) {
  try {
    ...
    if (forceDebug) {
      console.log(formatLogMessage('debug', `Loaded ${rules.size} comparison rules from ${compareFilePath}`));
    }
  ...
  for (const group of groups) {
    ...
    for (const rule of group.rules) {
      ...
      if (!comparisonRules.has(normalized)) {
        ...
        duplicateCount++;
        if (forceDebug) {
          console.log(formatLogMessage('debug', `Filtered duplicate rule: ${rule} (normalized: ${normalized})`));
        }
      }
    }
    ...
    if (group.title && filteredRules.length > 0) {
      ...
      orphanedTitles++;
      if (forceDebug) {
        console.log(formatLogMessage('debug', `Filtered orphaned title: ${group.title} (no unique rules remaining)`));
      }
    }
  }
  
  if (forceDebug) {
    console.log(formatLogMessage('debug', `Filtered ${duplicateCount} duplicate rules and ${orphanedTitles} orphaned titles`));
    console.log(formatLogMessage('debug', `${result.filter(r => !r.startsWith('!')).length} unique rules remaining`));
  }
