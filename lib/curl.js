...
async function downloadWithCurl(url, userAgent = '', options = {}) {
  ...
  // Confine BOTH the initial request and any redirect hop to http/https.
  // --proto is the load-bearing half: without it curl reads a `file://` initial
  // URL straight off disk (verified: file:///etc/passwd returns 1539 bytes).
  // --proto-redir mostly restates curl's default redirect protocol set —
  // measured on curl 8.5, file/dict/gopher/ldap/smb/scp/sftp/imap/tftp hops are
  // already refused, and it additionally blocks ftp/ftps.
  curlArgs.push('--proto', '=http,https', '--proto-redir', '=http,https');
...
function createCurlHandler(config) {
  ...
  return async function curlHandler(requestUrl) {
    try {
      ...
      // Previously this ran AFTER a URL parse + a detected-domain lookup,
