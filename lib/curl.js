  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code, signal) => {
      ...
      if (metadataParts.length !== CURL_DEFAULTS.METADATA_PIPE_PARTS) {
        resolve(errResult(`Invalid metadata format: expected ${CURL_DEFAULTS.METADATA_PIPE_PARTS} parts, got ${metadataParts.length}`));
        return;
      }
...
function createCurlHandler(config) {
  ...
  return async function curlHandler(requestUrl) {
    try {
      ...
      // Prepare custom headers from site config. Use getReferrerForUrl so
      // ALL referrer modes work — including object modes like
      // {mode: 'random_search'} that the old inline string/array logic
      // silently dropped (returned undefined for typeof !== 'string'),
      // making the Referer header inconsistent between browser navigation
      // and curl-side downloads of the same scan.
