...
function detectChallengeLoop(url, previousUrls = []) {
  ...
  // The `url.includes('/cdn-cgi/challenge-platform/')` conjunct that used
  // to live in here was redundant — the early-return above only lets us
  // through when url IS a challenge URL, so any prevUrl that also matches
  // the cdn-cgi path is by definition a "similar" challenge with a
  // different ray ID.
  ...
  const similarUrls = previousUrls.filter(prevUrl => {
    ...
    return urlIsCdnCgi && prevUrl.includes('/cdn-cgi/challenge-platform/');
  });
...
async function handleCloudflareProtection(page, currentUrl, siteConfig, forceDebug = false, navInfo = {}) {
  ...
  try {
    ...
    const handlingResult = await Promise.race([
      ...
      performCloudflareHandling(page, currentUrl, siteConfig, cfDebug, { cfBypassEnabled, cfPhishEnabled }).then(r => {
 ...
 */
async function performCloudflareHandling(page, currentUrl, siteConfig, forceDebug, flags) {
