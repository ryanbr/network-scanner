...
async function safeWaitForNavigation(page, timeout = TIMEOUTS.NAVIGATION_TIMEOUT) {
  ...
  try {
    ...
    console.warn(formatLogMessage('warn', `${CLOUDFLARE_TAG} Navigation wait failed: ${error.message}`));
  } finally {
    if (timeoutId) clearTimeout(timeoutId);
  }
}
...
async function handlePhishingWarning(page, currentUrl, forceDebug = false) {
  ...
  try {
    ...
    if (challengeInfo.isPhishingWarning) {
      ...
      if (forceDebug) {
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG} Phishing warning detected on ${currentUrl}:`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Page Title: "${challengeInfo.title}"`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Current URL: ${challengeInfo.url}`));
...
async function handleVerificationChallenge(page, currentUrl, forceDebug = false) {
  ...
  try {
    ...
    if (challengeInfo.isChallengePresent) {
      ...
      if (forceDebug) {
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG} Challenge detected on ${currentUrl}:`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Page Title: "${challengeInfo.title}"`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Current URL: ${challengeInfo.url}`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Is Turnstile: ${challengeInfo.isTurnstile}`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Is JS Challenge: ${challengeInfo.isJSChallenge}`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Has Legacy Checkbox: ${challengeInfo.hasLegacyCheckbox}`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Has Turnstile Iframe: ${challengeInfo.hasTurnstileIframe}`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Has Turnstile Container: ${challengeInfo.hasTurnstileContainer}`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Has Turnstile Checkbox: ${challengeInfo.hasTurnstileCheckbox}`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Has CAPTCHA: ${challengeInfo.hasCaptcha}`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Has Challenge Running: ${challengeInfo.hasChallengeRunning}`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Has Data Ray: ${challengeInfo.hasDataRay}`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Has Turnstile Response: ${challengeInfo.hasTurnstileResponse}`));
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG}   Body snippet: ${challengeInfo.bodySnippet}`));
      }
...
async function handleVerificationChallengeWithRetries(page, currentUrl, siteConfig, forceDebug = false) {
  ...
  for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
    try {
      ...
      // Check for redirect loops BEFORE pushing the current URL — otherwise
      // detectChallengeLoop's similarUrls filter counts the URL we just
      // pushed against itself, tripping the >=2 threshold one iteration
      // early (after 1 prior visit instead of the documented 2).
      ...
      // Push only AFTER the loop check passes so we never count the
      // current iteration against itself.
      visitedUrls.push(currentPageUrl);

      if (forceDebug && attempt > 1) {
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG} Challenge attempt ${attempt}/${retryConfig.maxAttempts} for ${currentUrl}`));
      }
      ...
      if (result.success || result.requiresHuman || !retryConfig.retryOnError) {
        if (forceDebug && attempt > 1) {
          console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG} Challenge ${result.success ? 'succeeded' : 'failed'} on attempt ${attempt}`));
        }
      ...
      if (attempt < retryConfig.maxAttempts) {
        ...
        if (forceDebug) {
          console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG} Challenge attempt ${attempt} failed, retrying in ${delay}ms: ${result.error}`));
        }
      ...
      if (forceDebug) {
        console.warn(formatLogMessage('warn', `${CLOUDFLARE_TAG} Challenge attempt ${attempt}/${retryConfig.maxAttempts} failed: ${error.message} [${errorType}]`));
      }
...
async function handlePhishingWarningWithRetries(page, currentUrl, siteConfig, forceDebug = false) {
  ...
  for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
    try {
      if (forceDebug && attempt > 1) {
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG} Phishing warning attempt ${attempt}/${retryConfig.maxAttempts} for ${currentUrl}`));
      }
      ...
      if (result.success || !retryConfig.retryOnError) {
        if (forceDebug && attempt > 1) {
          console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG} Phishing warning ${result.success ? 'succeeded' : 'failed'} on attempt ${attempt}`));
        }
        return { ...result, attempts: attempt };
      }
      ...
      if (attempt < retryConfig.maxAttempts) {
        const delay = getRetryDelay(attempt);
        if (forceDebug) {
          console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG} Phishing warning attempt ${attempt} failed, retrying in ${delay}ms: ${result.error}`));
        }
        await new Promise(resolve => setTimeout(resolve, delay));
      }
      ...
      if (forceDebug) {
        console.warn(formatLogMessage('warn', `${CLOUDFLARE_TAG} Phishing warning attempt ${attempt}/${retryConfig.maxAttempts} failed: ${error.message} [${errorType}]`));
      }
  ...
  const logOutcome = async (result) => {
    if (forceDebug) {
      try {
        ...
        console.log(formatLogMessage('debug', `${CLOUDFLARE_TAG} Outcome for ${currentUrl}: ${outcome} | ${clearanceTag} | ${bmTag}${statusTag}${rayTag} | duration=${Date.now() - startMs}ms`));
...
// checkChallengeCompletion, quickCloudflareDetection,
// enhancedParallelChallengeDetection, waitForTimeout, categorizeError,
// getRetryConfig, detectChallengeLoop, getModuleInfo, ERROR_TYPES,
// RETRY_CONFIG, CLOUDFLARE_MODULE_VERSION) stay as module-local functions —
// move them back to module.exports only if a new external consumer appears.
module.exports = {
