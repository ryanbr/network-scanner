...
function shouldProcessUrl(url, forceDebug = false) {
  if (!url || typeof url !== 'string') {
    if (forceDebug) console.log(formatLogMessage('cloudflare', `${URL_VALIDATION_TAG} Skipping invalid URL: ${url}`));
    return false;
  }
  ...
  if (skipMatch) {
    if (forceDebug) {
      console.log(formatLogMessage('cloudflare', `${URL_VALIDATION_TAG} Skipping ${skipMatch[0].toLowerCase()} URL: ${url.substring(0, 100)}${url.length > 100 ? '...' : ''}`));
    }
    return false;
  }
  ...
  if (!HTTP_PROTO_RE.test(url)) {
    if (forceDebug) {
      console.log(formatLogMessage('cloudflare', `${URL_VALIDATION_TAG} Skipping non-HTTP(S) URL: ${url.substring(0, 100)}${url.length > 100 ? '...' : ''}`));
    }
    return false;
  }
