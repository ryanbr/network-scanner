...
const aggregateStats = {
  ...
  bySolveMethod: Object.create(null),   // 'js_challenge_wait' -> N, 'turnstile' -> N, etc.
  totalDurationMs: 0,
...
function bumpAggregate(outcome, result, durationMs) {
  ...
  const m = result.verificationChallenge && result.verificationChallenge.method;
  if (m) aggregateStats.bySolveMethod[m] = (aggregateStats.bySolveMethod[m] || 0) + 1;
}
...
async function handleCloudflareProtection(page, currentUrl, siteConfig, forceDebug = false, navInfo = {}) {
  ...
  const logOutcome = async (result) => {
    ...
    // Always update aggregates regardless of debug mode -- so getAggregateStats()
    // returns useful end-of-scan numbers even in silent production runs.
    bumpAggregate(outcome, result, durationMs);

    const isFailure = !result.overallSuccess || result.timedOut;
    if (!forceDebug && !isFailure) return result; // production success path: silent

    try {
      const statusTag = navInfo.httpStatus != null ? ` | http=${navInfo.httpStatus}` : '';
      const rayTag = navInfo.cfRay ? ` | cf-ray=${navInfo.cfRay}` : '';
      const timingTag = ` | q=${timings.quick}ms p=${timings.phish}ms c=${timings.challenge}ms`;
      ...
      if (forceDebug) {
        ...
        // against CF's edge logs and identify slow stages.
        console.warn(formatLogMessage('warn', `${CLOUDFLARE_TAG} Outcome for ${currentUrl}: ${outcome}${statusTag}${rayTag} | duration=${durationMs}ms${timingTag}`));
      }
