...
async function quickCloudflareDetection(page, forceDebug = false) {
  try {
    ...
    const quickCheck = await safePageEvaluate(page, () => {
      ...
      // indicators found".
      if (/\|\s*5\d\d:/.test(title)) {
        return { hasIndicators: false, hasErrorPage: true, title, url, bodySnippet: '' };
      }
      ...
      const errorPageBody = /Error code 5\d\d/.test(bodyText);
      if (errorPageBody && !textMatch && !slowSelectorMatch) {
        return { hasIndicators: false, hasErrorPage: true, title, url, bodySnippet: bodyText.substring(0, 200) };
      }
    ...
    if (forceDebug) {
      ...
      } else if (quickCheck.hasErrorPage) {
        // Suppress the per-detection log when we also recognize this as a
        // 5xx error page — handleCloudflareProtection prints a clearer
        // "Cloudflare error page detected" line right after, and printing
        // both produces a contradictory "no indicators / error page detected"
        // pair for the same URL.
      } else {
