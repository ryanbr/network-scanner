...
async function handleCloudflareProtection(page, currentUrl, siteConfig, forceDebug = false) {
  ...
  // Trust the detection â€” explicit config only matters when indicators ARE found
  ...
  try {
    ...
    // Check if this domain already timed out â€” skip immediately
    try {
      ...
      if (cachedOutcome && cachedOutcome.data && cachedOutcome.data.timedOut && Date.now() - cachedOutcome.timestamp < detectionCache.ttl) {
        if (forceDebug) console.log(formatLogMessage('cloudflare', `Skipping ${currentUrl} â€” domain already timed out on a previous URL`));
