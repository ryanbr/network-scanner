...
function shouldDisableReferrer(targetUrl, disableList, forceDebug = false) {
  ...
  for (const disablePattern of disableList) {
    ...
    if (targetUrlParsed) {
      try {
        ...
        if (targetHostname === disableHostname) {
          if (forceDebug) console.log(formatLogMessage('debug', `${REFERRER_TAG} Referrer disabled for domain match: ${targetHostname}`));
          return true;
        }
      } catch (e) {
    ...
    // Boundary-aware hostname suffix match — only fires when the pattern
    // is a bare hostname (no '/') AND it matches as a complete hostname
    // OR a true subdomain suffix of the target's hostname. Replaces the
    // old plain substring match where 'example.com' wrongly matched
    // 'malicious-example.com' and 'example.community'.
    if (targetUrlParsed && !disablePattern.includes('/')) {
      const p = disablePattern.toLowerCase();
      const h = targetHostname.toLowerCase();
      if (h === p || h.endsWith('.' + p)) {
        if (forceDebug) console.log(formatLogMessage('debug', `${REFERRER_TAG} Referrer disabled for hostname suffix match: ${p} matches ${h}`));
        return true;
      }
    } else if (!targetUrlParsed) {
      // Target URL didn't parse — fall back to substring as last resort.
      // This only fires for malformed targets, which shouldn't happen in
      // practice (we only call this on URLs we're about to navigate to).
...
function generateReferrerUrl(referrerConfig, forceDebug = false) {
  try {
    ...
    if (forceDebug) console.log(formatLogMessage('debug', `${REFERRER_TAG} Referrer generation failed: ${err.message}`));
    return '';
  }
}
