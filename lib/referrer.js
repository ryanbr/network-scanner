...
function generateSearchReferrer(searchTerms, context, forceDebug) {
  ...
  if (forceDebug) {
    console.log(formatLogMessage('debug', `Generated search referrer: ${referrerUrl} (engine: ${randomEngine.split('//')[1].split('/')[0]}, term: "${searchTerm}")`));
  }
...
function generateSocialMediaReferrer(forceDebug) {
  ...
  if (forceDebug) {
    console.log(formatLogMessage('debug', `Generated social media referrer: ${randomSocial}`));
  }
...
function generateNewsReferrer(forceDebug) {
  ...
  if (forceDebug) {
    console.log(formatLogMessage('debug', `Generated news referrer: ${randomNews}`));
  }
...
function shouldDisableReferrer(targetUrl, disableList, forceDebug = false) {
  ...
  for (const disablePattern of disableList) {
    ...
    if (targetUrl === disablePattern) {
      if (forceDebug) console.log(formatLogMessage('debug', `Referrer disabled for exact match: ${targetUrl}`));
      return true;
    }
    ...
    if (targetUrlParsed) {
      try {
        ...
        if (targetHostname === disableHostname) {
          if (forceDebug) console.log(formatLogMessage('debug', `Referrer disabled for domain match: ${targetHostname}`));
          return true;
        }
      } catch (e) {
    ...
    if (!targetUrlParsed || disablePattern.includes('/') === false) {
      if (targetUrl.includes(disablePattern)) {
        if (forceDebug) console.log(formatLogMessage('debug', `Referrer disabled for pattern match: ${disablePattern} in ${targetUrl}`));
        return true;
      }
    }
  }
...
function generateReferrerUrl(referrerConfig, forceDebug = false) {
  try {
    ...
    if (typeof referrerConfig === 'string') {
      ...
      if (forceDebug && url) {
        console.log(formatLogMessage('debug', `Using direct referrer URL: ${url}`));
      } else if (forceDebug && !url) {
        console.log(formatLogMessage('debug', `Invalid referrer URL provided: ${referrerConfig}`));
      }
      return url;
    }
    ...
    if (Array.isArray(referrerConfig)) {
      if (referrerConfig.length === 0) {
        if (forceDebug) console.log(formatLogMessage('debug', `Empty referrer array provided`));
        return '';
      }
      ...
      if (forceDebug) {
        console.log(formatLogMessage('debug', `Selected referrer from array (${referrerConfig.length} options): ${url || 'invalid URL'}`));
      }
    ...
    if (typeof referrerConfig === 'object' && referrerConfig !== null && referrerConfig.mode) {
      switch (referrerConfig.mode) {
        ...
        case 'direct_navigation': {
          if (forceDebug) console.log(formatLogMessage('debug', `Using direct navigation (no referrer)`));
          return '';
        }
        ...
        case 'custom': {
          ...
          if (forceDebug) {
            console.log(formatLogMessage('debug', `Using custom referrer URL: ${url || 'invalid URL provided'}`));
          }
          return url;
        }
        ...
        case 'mixed': {
          ...
          if (forceDebug) console.log(formatLogMessage('debug', `Mixed mode selected: ${randomMode}`));
        ...
        default: {
          if (forceDebug) console.log(formatLogMessage('debug', `Unknown referrer mode: ${referrerConfig.mode}`));
          return '';
        }
      }
    }
    
    if (forceDebug) console.log(formatLogMessage('debug', `Invalid referrer configuration type: ${typeof referrerConfig}`));
    return '';
  } catch (err) {
    if (forceDebug) console.log(formatLogMessage('debug', `Referrer generation failed: ${err.message}`));
    return '';
  }
}
