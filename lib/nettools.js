...
async function digLookup(domain = '', recordType = 'A', timeout = 5000) {
  ...
  for (let i = 0; i < attemptPlan.length; i++) {
    const { server: digServerArg, tcp } = attemptPlan[i];
    ...
    // The TCP fallback is the last attempt and only runs once UDP has failed.
    // These flaky-link failures come in short bursts (both UDP and TCP time out
    // for a few seconds, then recover), so pause briefly before it to let the
    // burst clear — only on the already-failing path, never on a healthy lookup.
    if (tcp && i > 0) await new Promise(r => setTimeout(r, 400));
