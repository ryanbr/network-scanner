          ...
          if (!digResult) {
            ...
            if (pendingDigLookups.has(digCacheKey)) {
              ...
              if (digResult.success) {
                ...
                // fallback) without an answer — a flaky-resolver drop, not a
                // dead host. Counted + sampled by name so the end-of-run summary
                // can surface how many (and which) dig-gated matches the link
                // cost this run (not cached, so the next run retries).
                dnsCacheStats.digFailures++;
                pushFreshSample(dnsCacheStats.digFailedSamples, `${digDomain} (${digRecordType}${digResult.reason ? ', ' + digResult.reason : ''})`);
              }
