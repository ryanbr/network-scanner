...
const GLOBAL_DIG_CACHE_TTL = 10800000; // 3 hours (persisted to disk between runs)
...
const GLOBAL_WHOIS_CACHE_TTL = 14400000; // 4 hours (persisted to disk between runs)
...
function loadDiskCache(filePath, cache, ttl, maxSize) {
  try {
    ...
    // Corrupt or unreadable cache file — start fresh
  }
}
        ...
        if (!whoisResult) {
          ...
          try {
            ...
            if (whoisResult.success || 
              ...
              if (diskCacheEnabled) saveDiskCache(WHOIS_CACHE_FILE, globalWhoisResultCache, GLOBAL_WHOIS_CACHE_TTL, GLOBAL_WHOIS_CACHE_MAX);
      ...
      if (needsDigLookup) {
        ...
        try {
          ...
          digResult = await digLookup(digDomain, digRecordType, 5000); // 5 second timeout for dig
            ...
            // Cache the result for future use (in-memory + disk)
            globalDigResultCache.set(digCacheKey, {
              result: digResult,
              timestamp: now
            });
            if (diskCacheEnabled) saveDiskCache(DIG_CACHE_FILE, globalDigResultCache, GLOBAL_DIG_CACHE_TTL, GLOBAL_DIG_CACHE_MAX);
