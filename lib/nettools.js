...
// 28 days, persisted to disk between runs. Two things stay pinned for the whole
// window, so clear .digcache when either matters:
//   - A-records: a domain that moves off a matched IP range (e.g. leaves
//     Cloudflare) keeps answering from cache.
//   - NXDOMAIN: it arrives as success:true (a real answer, see the caching
//     comment in createNetToolsHandler), so a domain that is re-registered or
//     restored still reads as dead until the entry expires. Transient failures
//     (timeout/SERVFAIL/REFUSED) are success:false and are NOT cached, so only
//     the definitive "does not exist" answer persists.
const GLOBAL_DIG_CACHE_TTL = 2419200000;
const GLOBAL_DIG_CACHE_MAX = 10000; // headroom so large multi-URL runs don't evict hot entries mid-run (entries are tiny: a domain + short dig output)
...
const GLOBAL_WHOIS_CACHE_TTL = 129600000; // 36 hours (persisted to disk between runs). Registrar data is very stable and whois servers rate-limit aggressively, so caching long cuts repeat queries.
...
// User-maintained list of domains to skip dig confirmation on (.dnsignore) —
// e.g. known-dead cloak domains that SERVFAIL/NXDOMAIN every run and just add
// noise (they're already captured/listed and no longer resolve). One domain per
// line; blank lines and `#` comments ignored. A candidate is skipped if it
// equals, OR is a subdomain of, any entry (so `xptidujgjktk.com` also covers
// `www.xptidujgjktk.com`). Unlike ignoreDomains — which runs the dig and then
// drops the capture — this skips the dig ENTIRELY: no lookup, no failure count,
// not captured.
const DNS_IGNORE_FILE = path.join(__dirname, '..', '.dnsignore');
const dnsIgnoreSet = new Set();
function loadDnsIgnore() {
  try {
    if (!fs.existsSync(DNS_IGNORE_FILE)) return 0;
    dnsIgnoreSet.clear();
    for (let line of fs.readFileSync(DNS_IGNORE_FILE, 'utf8').split('\n')) {
      line = line.trim();
      if (!line || line.startsWith('#')) continue;
      dnsIgnoreSet.add(line.toLowerCase());
    }
    return dnsIgnoreSet.size;
  } catch (_) { return 0; }
}
function isDnsIgnored(name) {
  if (dnsIgnoreSet.size === 0 || !name) return false;
  const n = name.toLowerCase();
  if (dnsIgnoreSet.has(n)) return true;
  for (const e of dnsIgnoreSet) { if (n.endsWith('.' + e)) return true; }
  return false;
}

// Domains that failed dig this run with a DEAD signal (SERVFAIL/REFUSED — a
// resolver reached the authoritative NS and got nothing valid), candidates for
// auto-adding to .dnsignore under --dnsignore-auto. NOT timeout (flaky link,
// possibly live). Populated at the deferred failure-accounting point.
const digDeadCandidates = new Set();
// Append newly-seen dead domains to .dnsignore: only entries NOT already covered
// (equals or subdomain-of an existing line — no dupes), each stamped for
// traceability. Updates the in-memory set so the same run can't double-add.
function appendDnsIgnore() {
  const toAdd = [];
  for (const d of digDeadCandidates) {
    if (!isDnsIgnored(d)) { toAdd.push(d); dnsIgnoreSet.add(d.toLowerCase()); }
  }
  if (toAdd.length === 0) return { added: 0, domains: [] };
  try {
    const stamp = new Date().toISOString().slice(0, 10);
    fs.appendFileSync(DNS_IGNORE_FILE, `\n# auto-added ${stamp} (dead: SERVFAIL/REFUSED)\n${toAdd.join('\n')}\n`);
    return { added: toAdd.length, domains: toAdd };
  } catch (e) { return { added: 0, domains: [], error: e.message }; }
}
...
// cache TTL window (dig 28d / whois 36h). Populating on cache HITS (not just writes) handles
...
const dnsCacheStats = { digHits: 0, digMisses: 0, digFailures: 0, whoisHits: 0, whoisMisses: 0, freshDig: [], freshWhois: [], digFailedSamples: [] };
...
// Concurrency cap for the actual `dig` subprocess (counting semaphore, mirrors
// the DNS pre-check's in lib/dns.js). With a high max_concurrent the scanner can
// fire that many simultaneous dig lookups at the same handful of public
// resolvers; the burst gets rate-limited / dropped (and is rough on WSL2's
// UDP-through-NAT path). Capping in-flight digs paces the burst; excess callers
// queue and drain as slots free. Wraps ONLY the network call — cache hits and
// single-flight-deduped lookups return before ever acquiring a slot, so only
// genuine cache-miss lookups contend for it. Configurable via `dig_max_concurrent`
// (default 6); 0 or negative disables the cap.
let digMaxInFlight = 6;
let digInFlight = 0;
const digWaiters = [];
function setDigConcurrency(n) {
  const v = parseInt(n, 10);
  if (Number.isFinite(v)) digMaxInFlight = v; // <= 0 => uncapped (acquire is a no-op)
}
function acquireDigSlot() {
  if (digMaxInFlight <= 0) return Promise.resolve();
  return new Promise(resolve => {
    if (digInFlight < digMaxInFlight) { digInFlight++; resolve(); }
    else digWaiters.push(resolve);
  });
}
function releaseDigSlot() {
  if (digMaxInFlight <= 0) return;
  digInFlight--;
  if (digWaiters.length > 0 && digInFlight < digMaxInFlight) {
    digInFlight++;
    digWaiters.shift()();
  }
}

// Opt-in extra dig retries (dig_retry_failed). On a bursty resolver the first
// UDP+TCP pass can fail while a fresh attempt a few seconds later succeeds
// (verified: manual re-dig seconds later resolved a domain that just failed).
// When set, digLookup appends this many extra TCP attempts, each after a longer
// backoff, so a transient burst gets more chances to clear before the lookup
// gives up. Bounded and only reached on total failure; the recovered result
// flows through the normal match path (no end-of-run injection needed). 0 = off.
let digExtraRetries = 0;
function setDigExtraRetries(n) {
  const v = parseInt(n, 10);
  if (Number.isFinite(v) && v >= 0) digExtraRetries = Math.min(v, 5); // cap so a typo can't stall a run
}
// Backoff before each extra retry (dig_retry_backoff). Widen to cover longer
// resolver bursts — a short pause only clears a brief burst; a longer one waits
// out a resolver that stays flaky for tens of seconds. Default 3s, capped 60s.
let digRetryBackoffMs = 3000;
function setDigRetryBackoff(ms) {
  const v = parseInt(ms, 10);
  if (Number.isFinite(v) && v >= 0) digRetryBackoffMs = Math.min(v, 60000);
}
...
async function digLookup(domain = '', recordType = 'A', timeout = 5000) {
  ...
  // Attempt plan — each entry is { server, tcp }. UDP is tried first (fast on a
  // healthy link); a TCP fallback is appended LAST because TCP survives WSL2's
  // UDP-through-NAT packet drops. That drop is the `;; communications error ...
  // timed out` then a bare retry succeeds pattern: the UDP datagram vanishes
  // silently in the NAT layer, but a TCP connection retransmits and gets through.
  // Without the fallback the whole lookup fails (and success:false is NOT cached,
  // so the domain silently drops out of dig/dig-or matching for that run).
  const attemptPlan = [];
  if (usingResolvers) {
    // Pinned --dns resolvers: JS-owned UDP failover across up to 3 (each fast:
    // +time=2 +tries=1), then one TCP retry on the primary resolver.
    const n = Math.min(3, attempts.length);
    for (let i = 0; i < n; i++) attemptPlan.push({ server: attempts[i], tcp: false });
    attemptPlan.push({ server: attempts[0], tcp: true });
  } else {
    // System resolver: bare `dig name type` first (keeps dig's native
    // resolv.conf rotation + UDP retries), then a TCP fallback if that whole
    // burst was dropped.
    attemptPlan.push({ server: null, tcp: false });
    attemptPlan.push({ server: null, tcp: true });
  }
  // Opt-in extra retries (dig_retry_failed): more TCP attempts with a longer
  // backoff, so a bursty resolver gets extra chances to clear before we give up.
  for (let r = 0; r < digExtraRetries; r++) {
    attemptPlan.push({ server: usingResolvers ? attempts[0] : null, tcp: true, backoffMs: digRetryBackoffMs });
  }

  let lastError = 'no resolver attempts made';
  // Track the strongest failure signal seen across attempts so the caller can
  // tell a DEAD domain (SERVFAIL — a resolver reached the authoritative NS and
  // got a broken/no answer) from a FLAKY LINK (timeout — never got a reply) or
  // rate-limiting (REFUSED). SERVFAIL outranks REFUSED outranks timeout.
  let sawServfail = false, sawRefused = false;

  // Cap concurrent dig subprocesses (only genuine cache-miss lookups reach here —
  // cache hits and single-flight-deduped callers never acquire a slot). Released
  // in finally so every exit path (success return or fall-through) frees the slot.
  await acquireDigSlot();
  try {
  for (let i = 0; i < attemptPlan.length; i++) {
    const { server: digServerArg, tcp, backoffMs } = attemptPlan[i];
    // Build argv (execFile, no shell => args can't be injected):
    //   - system UDP: bare `name type`               (native retries preserved)
    //   - pinned UDP: `@srv +time=2 +tries=1 name type` (JS loop owns failover)
    //   - any TCP:    `[@srv] +tcp +time=3 +tries=2 name type` (WSL2-reliable)
    const digArgs = [];
    if (digServerArg) digArgs.push(digServerArg);
    if (tcp) digArgs.push('+tcp', '+time=3', '+tries=2');
    else if (digServerArg) digArgs.push('+time=2', '+tries=1');
    digArgs.push(cleanDomain, recordType);
    // TCP is slower — allow up to 8s; pinned UDP keeps its 4s ceiling; system
    // bare UDP keeps the full budget for dig's native retries.
    const perAttemptTimeout = tcp ? Math.min(Math.max(timeout, 6000), 8000)
      : usingResolvers ? Math.min(timeout, 4000)
      : timeout;
    const resolverLabel = (digServerArg ? digServerArg.slice(1) : 'system resolver') + (tcp ? '+tcp' : '');

    // Back off before a retry so a transient burst (both UDP and TCP time out
    // for a few seconds, then recover) has time to clear — only on the
    // already-failing path, never on a healthy lookup. The standard TCP fallback
    // uses a short 400ms pause; opt-in extra retries carry a longer backoffMs.
    const pauseMs = backoffMs != null ? backoffMs : (tcp && i > 0 ? 400 : 0);
    ...
    try {
      ...
      if (rcode === 'REFUSED' || rcode === 'SERVFAIL') {
        if (rcode === 'SERVFAIL') sawServfail = true; else sawRefused = true;
  ...
  // reason lets the summary flag likely-DEAD domains (SERVFAIL) apart from a
  // flaky link (timeout): SERVFAIL from a public resolver means it reached the
  // authoritative NS and got nothing valid — the domain, not our link, is broken.
  const reason = sawServfail ? 'SERVFAIL' : sawRefused ? 'REFUSED' : 'timeout';
  return {
    success: false,
    error: lastError,
    reason,
...
function createNetToolsHandler(config) {
  ...
  return async function handleNetToolsCheck(domain, fullSubdomain) {
    ...
    // .dnsignore: skip the dig for known-dead domains (equals or subdomain-of an
    // entry) BEFORE any lookup — no SERVFAIL noise, no failure count, not captured.
    const digIsIgnored = (hasDig || hasDigOr) && isDnsIgnored(digDomain);
    if (digIsIgnored && forceDebug) {
      logToConsoleAndFile(`${messageColors.highlight('[dig]')} Skipping ${digDomain} — matched .dnsignore`);
    }
    const needsDigLookup = (hasDig || hasDigOr) && !processedDigDomains.has(digDedupeKey) && !digIsIgnored;
    ...
    try {
      ...
      if (needsDigLookup) {
        ...
        try {
          // Holds the root-dig failure sample until AFTER the subdomain fallback
          // below — so a candidate recovered on its real requested name isn't
          // counted/reported as a failure. Null unless the fresh root lookup failed.
          let digFailPending = null;
          ...
          if (!digResult) {
            ...
            if (pendingDigLookups.has(digCacheKey)) {
              ...
              // cache TTL (28d when persisted via --dns-cache), so a host that
              ...
              if (digResult.success) {
                ...
                // Fresh lookup exhausted every attempt (UDP failover + TCP
                // fallback) without an answer. DEFER counting it until after the
                // subdomain fallback — a candidate whose apex is dead but whose
                // requested subdomain resolves must not be reported as a failure.
                digFailPending = {
                  sample: `${digDomain} (${digRecordType}${digResult.reason ? ', ' + digResult.reason : ''})`,
                  domain: digDomain,
                  reason: digResult.reason
                };
              }
          ...
          // Subdomain fallback: the root-dig above didn't confirm, but the
          // request was to a DISTINCT subdomain whose DNS can differ from a
          // dead/parked apex (SERVFAIL or no-A at the root, live on the sub).
          // Only in default root-dig mode (digSubdomain:false). Re-dig the exact
          // requested name before giving up, so a subdomain-served ad domain
          // isn't dropped and the failure report doesn't mislabel it dead.
          if ((hasDig || hasDigOr) && !digMatched && !digOrMatched && !digSubdomain
              && originalDomain && originalDomain !== digDomain && !isDnsIgnored(originalDomain)) {
            const subKey = `${originalDomain}-${digRecordType}`;
            const subNow = Date.now();
            let subRes = null;
            if (globalDigResultCache.has(subKey)) {
              const e = globalDigResultCache.get(subKey);
              if (subNow - e.timestamp < GLOBAL_DIG_CACHE_TTL) { subRes = e.result; dnsCacheStats.digHits++; }
              else globalDigResultCache.delete(subKey);
            }
            if (!subRes) {
              if (pendingDigLookups.has(subKey)) {
                subRes = await pendingDigLookups.get(subKey);
              } else {
                const subPromise = digLookup(originalDomain, digRecordType, 5000);
                pendingDigLookups.set(subKey, subPromise);
                try { subRes = await subPromise; } finally { pendingDigLookups.delete(subKey); }
                if (subRes.success) globalDigResultCache.set(subKey, { result: subRes, timestamp: subNow, hostname: originalDomain });
                dnsCacheStats.digMisses++;
                pushFreshSample(dnsCacheStats.freshDig, `${originalDomain} (${digRecordType})`);
                if (subRes.success && digOutputIndicatesResolution(subRes.output)) markResolved(originalDomain);
              }
            }
            if (subRes && subRes.success) {
              const subLo = subRes.output.toLowerCase();
              if (hasDig) digMatched = digTermsLower.every(t => subLo.includes(t));
              if (hasDigOr) digOrMatched = digOrTermsLower.some(t => subLo.includes(t));
              if (digMatched || digOrMatched) {
                digFailPending = null; // recovered on the real requested name — not a failure
                // Mirror the primary dry-run callback so a subdomain-recovered
                // match is recorded in dry-run output, not just on real runs.
                if (dryRunCallback) {
                  if (hasDig && digMatched) {
                    dryRunCallback(domain, 'dig', 'AND logic', digTerms.join(', '), `All terms found in ${digRecordType} records (via subdomain ${originalDomain})`, { queriedDomain: originalDomain, recordType: digRecordType, subdomainMode: true });
                  }
                  if (hasDigOr && digOrMatched) {
                    const oi = digOrTermsLower.findIndex(t => subLo.includes(t));
                    dryRunCallback(domain, 'dig', 'OR logic', digOrTerms[oi], `Term found in ${digRecordType} records (via subdomain ${originalDomain})`, { queriedDomain: originalDomain, recordType: digRecordType, subdomainMode: true });
                  }
                }
                if (forceDebug) logToConsoleAndFile(`${messageColors.highlight('[dig]')} Root ${digDomain} didn't confirm; recovered via requested subdomain ${originalDomain}`);
              }
            }
          }

          // Deferred failure accounting: only now — after the subdomain fallback
          // had its chance — do we know the candidate is truly unconfirmable.
          if (digFailPending) {
            dnsCacheStats.digFailures++;
            pushFreshSample(dnsCacheStats.digFailedSamples, digFailPending.sample);
            // Only a DEAD signal (SERVFAIL/REFUSED) is an auto-.dnsignore
            // candidate; a timeout is flaky-link (possibly live) — never auto-add.
            if (digFailPending.reason === 'SERVFAIL' || digFailPending.reason === 'REFUSED') {
              digDeadCandidates.add(digFailPending.domain);
            }
          }
        } catch (digError) {
          if (forceDebug) {
...
module.exports = {
  ...
  setDigResolvers,
  // Cap concurrent dig subprocesses (dig_max_concurrent; default 6, 0 = uncapped).
  setDigConcurrency,
  // Extra TCP dig retries on total failure (dig_retry_failed; default 0 = off).
  setDigExtraRetries,
  // Backoff before each extra retry (dig_retry_backoff; default 3000ms, cap 60s).
  setDigRetryBackoff,
  ...
  loadDnsIgnore,
  // Append this run's newly-detected dead (SERVFAIL/REFUSED) domains to
  // .dnsignore, deduped — for --dnsignore-auto.
  appendDnsIgnore,
