...
function createNetToolsHandler(config) {
  ...
  return async function handleNetToolsCheck(domain, originalDomain) {
    try {
      ...
      if (hasDig) {
        if (forceDebug) {
          console.log(`[debug][dig] Performing dig lookup for ${digDomain} (${digRecordType})${digSubdomain ? ' [subdomain mode]' : ''}`);
        }
        
        const digResult = await digLookup(digDomain, digRecordType);
        
        if (digResult.success) {
          digMatched = checkDigTerms(digResult.output, digTerms);
          
          if (forceDebug) {
            console.log(`[debug][dig] Lookup completed for ${digDomain}, matched: ${digMatched}`);
      ...
      if (shouldMatch) {
        ...
        if (siteConfig.verbose === 1) {
          ...
          if (hasDig && digMatched) matchType.push(digSubdomain ? 'dig-subdomain' : 'dig');
