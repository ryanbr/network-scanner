...
const GLOBAL_DIG_CACHE_TTL = 129600000; // 36 hours (persisted to disk between runs). Note: A-records can change, so a domain that moves off a matched IP range (e.g. leaves Cloudflare) can stay cached up to this long.
const GLOBAL_DIG_CACHE_MAX = 4000; // headroom so large multi-URL runs don't evict hot entries mid-run (entries are tiny: a domain + short dig output)
        ...
        try {
          ...
          if (!digResult) {
            ...
            if (pendingDigLookups.has(digCacheKey)) {
              ...
              // cache TTL (36h when persisted via --dns-cache), so a host that
