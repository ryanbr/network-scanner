...
async function whoisLookup(domain, timeout = 10000, whoisServer = null, debugMode = false) {
  ...
  try {
    ...
    if (debugMode) {
      console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Adding 2 second delay to prevent rate limiting...`));
    }
    
    // Add 2 second delay to prevent "access too fast" rate limiting
...
async function whoisLookupWithRetry(domain, timeout = 10000, whoisServer = null, debugMode = false, retryOptions = {}) {
  ...
  for (const server of serversToTry) {
    ...
    if (attemptCount > 1) {
      if (debugMode) {
        console.log(formatLogMessage('debug', `${messageColors.highlight('[whois-retry]')} Adding 2 second delay before retry attempt...`));
      }
      await new Promise(resolve => setTimeout(resolve, 2000));
   }
  ...
  return async function handleNetToolsCheck(domain, originalDomain) {
    ...
    try {
      ...
      if (hasWhois || hasWhoisOr) {
        ...
        // Add delay before whois lookup in nettools handler to prevent rate limiting
        if (forceDebug) {
          console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Adding 2 second delay to prevent rate limiting...`));
        }
        await new Promise(resolve => setTimeout(resolve, 2000));
        ...
        if (forceDebug) {
          console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Starting whois lookup after delay...`));
        }
        ...
        try {
          ...
          if (whoisResult.success) {
            ...
            if (forceDebug) {
              ...
              console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Lookup completed for ${domain}${serverUsed} in ${whoisResult.duration}ms${retryInfo}`));
              
              
              if (whoisResult.retryInfo && whoisResult.retryInfo.retriedAfterFailure) {
                console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Success after retry - servers attempted: [${whoisResult.retryInfo.serversAttempted.map(s => s || 'default').join(', ')}]`));
              }
            }
          } else {
            ...
            if (forceDebug) {
              ...
              console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} ${errorContext}: Lookup failed for ${domain}${serverUsed} after ${whoisResult.duration}ms${retryInfo}`));
              console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Command executed: ${whoisResult.command || 'unknown'}`));
              console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Error details: ${whoisResult.error}`));
              
              // Enhanced server debugging for failures
              if (whoisResult.whoisServer) {
                console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Failed server: ${whoisResult.whoisServer} (custom)`));
              } else {
                console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Failed server: system default whois server`));
              }
              ...
              if (whoisResult.retryInfo) {
                if (whoisResult.retryInfo.allAttemptsFailed) {
                  console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} All retry attempts failed. Servers tried: [${whoisResult.retryInfo.serversAttempted.map(s => s || 'default').join(', ')}]`));
                }
                
                if (whoisResult.retryInfo.retriedAfterFailure) {
                  console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Retries were attempted but ultimately failed`));
                }
              }
              
              if (whoisResult.isTimeout) {
                console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Timeout exceeded limit after all retry attempts`));
                if (Array.isArray(whoisServer) && whoisServer.length > 1) {
                  ...
                  if (remainingServers.length > 0) {
                    console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Unused servers from config: ${remainingServers.join(', ')}`));
                  }
                } else {
                  ...
                  if (suggestions.length > 0) {
                    console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Suggested alternative servers: ${suggestions.join(', ')}`));
                  }
                }
                ...
                if (whoisResult.error.toLowerCase().includes('too fast') || whoisResult.error.toLowerCase().includes('rate limit')) {
                  console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Rate limiting detected - consider increasing delays or using different servers`));
                  console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Current server: ${whoisResult.whoisServer || 'default'} may be overloaded`));
                }
              }
              
              // Log specific error patterns
              if (whoisResult.error.toLowerCase().includes('connection refused')) {
                console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Connection refused - server may be down or blocking requests`));
              } else if (whoisResult.error.toLowerCase().includes('no route to host')) {
                console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Network connectivity issue to whois server`));
              } else if (whoisResult.error.toLowerCase().includes('name or service not known')) {
                console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} DNS resolution failed for whois server`));
              }
            }
          ...
          if (forceDebug) {
            console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Exception during lookup for ${domain}: ${whoisError.message}`));
            console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Exception type: ${whoisError.constructor.name}`));
            if (whoisError.stack) {
              console.log(formatLogMessage('debug', `${messageColors.highlight('[whois]')} Stack trace: ${whoisError.stack.split('\n').slice(0, 3).join(' -> ')}`));
            }
          }
      ...
      if (hasDig || hasDigOr) {
        ...
        try {
          ...
          if (digResult.success) {
            ...
            if (forceDebug) {
              console.log(formatLogMessage('debug', `${messageColors.highlight('[dig]')} Lookup failed for ${digDomain}: ${digResult.error}`));
            }
          }
        } catch (digError) {
          if (forceDebug) {
            console.log(formatLogMessage('debug', `${messageColors.highlight('[dig]')} Exception during lookup for ${digDomain}: ${digError.message}`));
          }
        }
      }
      ...
      if (forceDebug) {
        console.log(formatLogMessage('debug', `${messageColors.highlight('[nettools]')} Error processing ${domain}: ${error.message}`));
      }
