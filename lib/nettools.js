...
function nextDigServerArg() {
  if (digResolvers.length === 0) return null;
  return '@' + digResolvers[digResolverCursor++ % digResolvers.length];
}
...
async function digLookup(domain = '', recordType = 'A', timeout = 5000) {
  try {
    ...
    // any chars without injection risk. When --dns is set, prepend `@<resolver>`
    // (round-robin) so dig uses the reliable servers instead of resolv.conf.
    const digServerArg = nextDigServerArg();
    const digArgs = digServerArg ? [digServerArg, cleanDomain, recordType] : [cleanDomain, recordType];
    const { stdout: fullOutput, stderr } = await execFileWithTimeout('dig', digArgs, timeout);
