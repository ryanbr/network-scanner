...
// Subsystem tag prefix for all log lines in this module. Matches the
// per-module tagging convention used across lib/ (see socks-relay.js,
// cloudflare.js, adblock.js, etc.) so mixed output stays grep-able by
// `[proxy]`. The 'proxy' severity in formatLogMessage drives the color
// theme; the tag is what the reader actually scans for.
...
async function prepareSocksRelays(siteConfigs, forceDebug = false) {
  ...
  for (const cfg of (siteConfigs || [])) {
    ...
    try {
      ...
      console.warn(formatLogMessage('proxy', `${PROXY_TAG} Failed to start SOCKS5 auth relay for ${parsed.host}:${parsed.port}: ${e.message}`));
    }
  }
  return started;
}
...
function getProxyArgs(siteConfig, forceDebug = false) {
  ...
  if (!parsed) {
    console.warn(formatLogMessage('proxy', `${PROXY_TAG} Invalid proxy URL: ${proxyUrl}`));
    return [];
  }
  ...
  if (parsed.protocol === 'socks5' && parsed.username) {
    ...
    if (relayPort) {
      ...
      if (debug) {
        console.log(formatLogMessage('proxy', `${PROXY_TAG} SOCKS5 auth via local relay 127.0.0.1:${relayPort} -> ${parsed.host}:${parsed.port}`));
      }
    } else {
      // prepareSocksRelays should have started this; defensive only.
      console.warn(formatLogMessage('proxy', `${PROXY_TAG} No SOCKS5 auth relay for ${parsed.host}:${parsed.port} — call prepareSocksRelays() before the scan. Connection will fail (Chromium can't auth SOCKS).`));
    }
  }
  ...
  if (parsed.protocol === 'socks5' && remoteDns !== false) {
    ...
    console.warn(formatLogMessage('proxy', `${PROXY_TAG} proxy_remote_dns ignored: SOCKS4 cannot do proxy-side DNS resolution (use SOCKS5)`));
  }
  ...
  const debug = forceDebug || siteConfig.proxy_debug || siteConfig.socks5_debug;
  if (debug) {
    console.log(formatLogMessage('proxy', `${PROXY_TAG} [${parsed.protocol}] Args: ${args.join(' ')}`));
  }
...
async function applyProxyAuth(page, siteConfig, forceDebug = false) {
  ...
  if (parsed.protocol === 'socks4') {
    console.warn(formatLogMessage('proxy', `${PROXY_TAG} SOCKS4 proxy auth is unsupported (use SOCKS5, which is auto-relayed, or an authenticated HTTP proxy).`));
    return false;
  }
  ...
  try {
    ...
    if (debug) {
      ...
      console.log(formatLogMessage('proxy', `${PROXY_TAG} Auth set for [redacted]@${parsed.host}:${parsed.port}`));
    }

    return true;
  } catch (err) {
    console.warn(formatLogMessage('proxy', `${PROXY_TAG} Failed to set proxy auth: ${err.message}`));
    return false;
  }
}
...
// getModuleInfo() / PROXY_MODULE_VERSION / SUPPORTED_PROTOCOLS removed
// from exports -- zero external callers (mirrors the same trim done in
// lib/cloudflare.js). SUPPORTED_PROTOCOLS stays as a module-local const
// since parseProxyUrl uses it.
