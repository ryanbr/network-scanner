...
// Update when adding new siteConfig fields anywhere in the scan path.
const KNOWN_SITE_CONFIG_KEYS = new Set([
  ...
  'cloudflare_bypass', 'cloudflare_max_retries',
  ...
  'delay_uncapped', 'detect_js_patterns', 'dig', 'digRecordType', 'dig_subdomain',
  ...
  'flowproxy_delay', 'flowproxy_js_timeout', 'flowproxy_nav_timeout',
  'flowproxy_page_timeout', 'forcereload', 'ghost_cursor_duration',
  'ghost_cursor_hesitate', 'ghost_cursor_overshoot', 'ghost_cursor_speed',
  'goto_options', 'grep', 'ignore_similar', 'ignore_similar_ignored_domains',
  ...
  'whois', 'whois_delay', 'whois_max_retries', 'whois_retry_on_error',
...
const BOOLEAN_SITE_CONFIG_FIELDS = new Set([
  ...
  'disable_adblock', 'dnsmasq', 'dnsmasq_old', 'evaluateOnNewDocument',
  'firstParty', 'grep', 'ignore_similar', 'ignore_similar_ignored_domains',
...
// Suggest a known key for an unknown one. Threshold scales with the unknown
// key's length (40%, min 2) so 'whois_terms' (11 chars) accepts edits up to
// distance 4 — comfortably catches the whois_terms → whois case (distance 6
// actually, but the prefix match makes it the best candidate anyway). For
// shorter typos like 'intract' → 'interact' (distance 1) the min-2 floor
// keeps a one-char typo matchable.
...
function normalizeSiteConfig(siteConfig, siteIndex = 0) {
  ...
  // 3. Dependent-flag implication: clear_sitedata_full_on_reload only takes
