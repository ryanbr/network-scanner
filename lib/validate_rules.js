...
function validateSiteConfig(siteConfig, siteIndex = 0) {
  ...
  // Validate blocked patterns. A bare string is accepted and treated as a
  // single pattern, same as filterRegex above and same as the coercion
  // normalizeSiteConfig() applies at scan startup -- otherwise
  // --validate-config would reject a config the scanner happily runs.
  if (siteConfig.blocked) {
    if (typeof siteConfig.blocked !== 'string' && !Array.isArray(siteConfig.blocked)) {
      errors.push(`Site ${siteIndex}: 'blocked' must be a string or an array of strings`);
    } else {
      const blockedPatterns = Array.isArray(siteConfig.blocked)
        ? siteConfig.blocked
        : [siteConfig.blocked];
      blockedPatterns.forEach((pattern, patternIndex) => {
        const validation = validateRegexPattern(pattern);
        if (!validation.isValid) {
  ...
  // Validate resource types. A bare string is one type, matching the coercion
  // normalizeSiteConfig() applies at scan startup, so --validate-config cannot
  // reject a config the scanner runs.
  if (siteConfig.resourceTypes) {
    if (typeof siteConfig.resourceTypes !== 'string' && !Array.isArray(siteConfig.resourceTypes)) {
      errors.push(`Site ${siteIndex}: 'resourceTypes' must be a string or an array of strings`);
    } else {
      const validTypes = ['script', 'stylesheet', 'image', 'font', 'document', 'subdocument', 'xhr', 'fetch', 'websocket', 'media', 'ping', 'other'];
      const declaredTypes = Array.isArray(siteConfig.resourceTypes)
        ? siteConfig.resourceTypes
        : [siteConfig.resourceTypes];
  ...
  // Validate CSS selectors. A bare string is one selector, matching the
  // coercion normalizeSiteConfig() applies at scan startup.
  if (siteConfig.css_blocked) {
    if (typeof siteConfig.css_blocked !== 'string' && !Array.isArray(siteConfig.css_blocked)) {
      errors.push(`Site ${siteIndex}: 'css_blocked' must be a string or an array of strings`);
    }
    // Note: CSS selector validation would be complex, skipping for now
  }
  
  // Validate numeric fields
  // js_redirect_timeout and max_redirects were absent, so degenerate values went
  // through silently. lib/redirect.js derives its poll interval as
  // js_redirect_timeout / 3, so a negative value produces a negative interval and
  // a non-number produces NaN -- setTimeout treats both as "fire immediately", so
  // the loop spins its three polls with no wait and JS redirects stop being
  // tracked at all, with nothing said. max_redirects is better protected (that
  // module does its own `typeof === 'number' && >= 0` check and falls back to 10),
  // so this is for the error message rather than for safety.
  //
  // Zero is accepted for all of these: max_redirects: 0 legitimately means "track
  // no redirects". The message says ">= 0" rather than "positive" because that is
  // what the check actually enforces.
  const numericFields = ['delay', 'reload', 'timeout', 'js_redirect_timeout', 'max_redirects'];
  numericFields.forEach(field => {
    if (siteConfig[field] !== undefined) {
      if (typeof siteConfig[field] !== 'number' || siteConfig[field] < 0) {
        errors.push(`Site ${siteIndex}: '${field}' must be a number >= 0`);
      }
    }
  });
  
  // Validate boolean fields
  const booleanFields = ['interact', 'clear_sitedata', 'firstParty', 'thirdParty', 'screenshot', 'headful', 'ignore_similar', 'ignore_similar_ignored_domains', 'capture_popups_signal'];
 ...
 if (siteConfig.ignore_similar_threshold !== undefined) {
  ...
  // Validate user agent
  // cookies: validate via the SAME normalizer the runtime uses, so
  // --validate-config cannot disagree with what a scan actually accepts.
  if (siteConfig.cookies !== undefined && siteConfig.cookies !== null) {
    const { normalizeCookies } = require('./cookies');
    // Probe with a REAL url from the config: the default cookie scope is derived
    // from the host (see lib/site-scope.js), and so is the host-only-domain
    // warning, so a placeholder host would silence a warning the scan then
    // raises. Falls back to the first entry of a url array for the same reason.
    const probeUrl = typeof siteConfig.url === 'string'
      ? siteConfig.url
      : (Array.isArray(siteConfig.url) && typeof siteConfig.url[0] === 'string'
        ? siteConfig.url[0]
        : 'https://example.invalid/');
    const { cookies: okCookies, errors: cookieErrors } = normalizeCookies(siteConfig.cookies, probeUrl);
    for (const e of cookieErrors) {
      warnings.push(`Site ${siteIndex}: cookies — ${e}`);
    }
    if (okCookies.length === 0 && cookieErrors.length > 0) {
      errors.push(`Site ${siteIndex}: 'cookies' is set but no usable cookie could be built from it`);
    }
  }

  // local_storage / session_storage: same principle as cookies above -- validate
  // through the runtime's own normalizer so --validate-config cannot disagree
  // with what a scan accepts.
  for (const optionName of ['local_storage', 'session_storage']) {
    const raw = siteConfig[optionName];
    if (raw === undefined || raw === null) continue;
    const { normalizeStorageItems } = require('./storage');
    const { items, errors: storageErrors } = normalizeStorageItems(raw, optionName);
    for (const e of storageErrors) {
      warnings.push(`Site ${siteIndex}: ${optionName} — ${e}`);
    }
    if (items.length === 0 && storageErrors.length > 0) {
      errors.push(`Site ${siteIndex}: '${optionName}' is set but no usable entry could be built from it`);
    }
  }
...
const KNOWN_SITE_CONFIG_KEYS = new Set([
  ...
  'capture_popups_max_depth', 'capture_popups_signal', 'capture_popups_window_ms',
  'cdp', 'cdp_specific',
  ...
  'interact_popups',
  'js_redirect_timeout', 'local_storage', 'localhost', 'max_redirects', 'openvpn', 'pihole',
  'output_regex',
...
const BOOLEAN_SITE_CONFIG_FIELDS = new Set([
  'adblock_rules', 'bypass_cache', 'capture_popups', 'capture_popups_signal',
  'cdp', 'clear_sitedata',
  ...
  'interact', 'interact_clicks', 'interact_popups', 'interact_scrolling', 'isBrave', 'localhost',
...
const STRING_TO_ARRAY_FIELDS = new Set([
  ...
  // 'blocked' has the identical silent-drop shape: compilePatternList() gates on
  // Array.isArray() and returns [] for anything else, so a bare string pattern
  // never blocked anything and nothing said so at scan time. It is also the odd
  // one out among the pattern fields -- filterRegex, searchstring, comments, url
  // and referrer_headers all accept either form.
  'blocked',
  // Same shape again: nwss.js builds allowedResourceTypesSet only when
  // Array.isArray() passes and otherwise leaves it null, which means NO
  // resource-type filtering at all -- so `resourceTypes: "image"` quietly
  // processed every type instead of only images.
  'resourceTypes',
  // Same silent-drop shape, found by sweeping every Array.isArray(siteConfig.X)
  // gate. All three are per-site only (no global equivalent), so coercing here
  // covers every consumer -- no need to duplicate it at the call sites the way
  // `blocked` requires for its global list.
  //   click_elements: `Array.isArray(...) && length > 0` at nwss.js:3667/5168,
  //     so a string skipped the clicking step entirely.
  //   cdp_specific:   nwss.js:2541 falls through to the else, so per-domain CDP
  //     logging never enabled.
  //   css_blocked:    nwss.js:3032 skips, so a string hid nothing. Note that
  //     guard is load-bearing, not redundant: selectors.map() over the string
  //     "#ad" would iterate PER CHARACTER and inject `a { display: none }`,
  //     hiding every link on the page. Coercion keeps it an array of one.
  'click_elements', 'cdp_specific', 'css_blocked',
]);
