...
/**
 * Render a list of origins for a log line without letting it run away. A site
 * whose URLs land on many subdomains accumulates one origin each in _seedOrigins,
 * and every one of them can end up unreachable at teardown, so the untruncated
 * list could be thousands of characters of warning.
 */
function originList(origins) {
  if (!origins.length) return 'any known origin';
  if (origins.length <= 3) return origins.join(', ');
  return `${origins.slice(0, 3).join(', ')} and ${origins.length - 3} more`;
}
...
async function removeStorage(page, items, scope, forceDebug = false) {
  ...
  try {
    ...
    if (cleared.size > 0) {
      ...
      if (forceDebug) {
        console.log(formatLogMessage('debug', `${STORAGE_TAG} Nothing to clear for ${site}: no page remains on ${originList(unreachable)} and this URL did not end on the site, so the entries were most likely never written`));
      }
    } else {
      ...
      console.log(formatLogMessage('warn', `${STORAGE_TAG} Could not clear ${releasable.length} seeded localStorage key(s) for ${site}: no live page remains on ${originList(unreachable)}; a later URL on this host may inherit them`));
    }
