...
async function removeStorage(page, items, scope, forceDebug = false) {
  ...
  try {
    ...
    if (cleared.size > 0) {
      ...
      // Nothing could be cleared anywhere. The refcount is already gone, so these
      // keys now outlive the entry and a later URL on this host inherits them --
      // warn rather than leave it to a --debug rerun to notice.
