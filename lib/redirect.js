  ...
  const navigationHandler = (frame) => {
    if (frame === page.mainFrame()) {
      ...
      if (frameUrl && frameUrl !== 'about:blank' && !frameUrl.startsWith('chrome-error://') && !redirectChainSet.has(frameUrl)) {
        ...
        //
        // NOTE the off-by-one, which the docs used to get wrong: redirectChain is
        // seeded with the ORIGINAL url, so its length is already 1 here before
        // any redirect has been recorded. `>= maxRedirects` therefore bails
        // immediately for both 0 AND 1 -- they behave identically -- and a value
        // of N tracks N-1 redirects (default 10 tracks nine).
        //
        ...
        // 302: at 0 and 1 the landed page is still loaded and its requests are
        // still captured.
