...
async function navigateWithRedirectHandling(page, currentUrl, siteConfig, gotoOptions = {}, forceDebug = false, formatLogMessage) {
  ...
  const jsRedirectDetector = async () => {
    try {
      ...
      await page.evaluateOnNewDocument((maxWaitMs) => {
        ...
        try {
          const noteMetaRefresh = (content) => {
            if (!content) return;
            // Only count a refresh that can actually fire inside the wait window.
            // The flag keeps the poll loop alive for its full budget, so flagging
            // every refresh made pages carrying a long one -- `content="3600"` is
            // a real cache/poll-hint pattern -- pay the whole wait for a redirect
            // an hour away: measured 4606ms -> 6601ms at a 3000ms budget before
            // this check. A delay that cannot land in time is not pending.
            //
            // An unparseable delay is treated as immediate, which is what browsers
            // do, so it still counts.
            const delaySec = parseInt(String(content).trim(), 10);
            if (Number.isFinite(delaySec) && delaySec * 1000 > maxWaitMs) return;

            // Store an actual URL, not the raw content attribute. The field is
            // named _jsRedirectUrl and was being handed things like
            // "2;url=/landed.html" or bare "3", which is not a URL by any reading.
            //
            // Content syntax is `<delay>` or `<delay>;url=<target>`, where the
            // separator is case-insensitive and the target may be quoted. With no
            // url= part a meta refresh reloads the current document, so that is
            // the target. Relative targets resolve against document.baseURI so a
            // <base href> is honoured, and an unresolvable target falls back to
            // the raw string rather than being dropped.
            let target = '';
            const urlPart = /;\s*url\s*=\s*([\s\S]*)$/i.exec(String(content));
            if (urlPart) {
              target = urlPart[1].trim();
              const q = target.charAt(0);
              if ((q === '"' || q === "'") && target.charAt(target.length - 1) === q) {
            ...
            if (!target) {
              ...
              try {
                resolved = new URL(target, document.baseURI).href;
              } catch (_) {
                resolved = target;
              }
            }
            ...
            window._jsRedirectUrl = resolved;
  ...
  try {
    ...
    // Same shape as truncateUrl() in lib/dry-run.js (80 chars, '...'), duplicated
    // rather than imported: that one is an internal helper and is not exported,
    // and this module deliberately has no requires at all -- it takes
    // formatLogMessage as a parameter for exactly that reason.
    //
    // Needed because a meta refresh can target a data: URL, which resolves to an
    // arbitrarily long string that then lands in a debug line verbatim.
    const forLog = (url, maxLength = 80) => {
    ...
    while (jsRedirectTimeout > 0 && pendingRedirectRechecks < JS_REDIRECT_POLLS) {
      ...
      try {
        ...
        if (currentPageUrl && currentPageUrl !== finalUrl && !currentPageUrl.startsWith('chrome-error://') && !redirectChainSet.has(currentPageUrl)) {
          ...
          if (forceDebug) {
            if (jsRedirectResult.detected) {
              // Show the detected TARGET and where it actually landed when they
              // differ -- the target is what the page asked for, the landed URL is
              // what committed, and a mismatch is worth seeing (a further hop, or
              // a target the browser declined).
              const target = jsRedirectResult.url;
              const where = (target && target !== currentPageUrl)
                ? `${forLog(target)} (landed on ${forLog(currentPageUrl)})`
                : forLog(currentPageUrl);
              console.log(formatLogMessage('debug', `JavaScript redirect detected (${jsRedirectResult.type}): ${where}`));
            } else {
        ...
        if (jsRedirectResult.detected && !redirected) {
          if (forceDebug) {
            // Include the target here too: this branch fires BEFORE the navigation
            // commits, so it is the one place the destination is known in advance,
            // and it was the only detection log omitting it.
            const pendingTarget = jsRedirectResult.url ? ` -> ${forLog(jsRedirectResult.url)}` : '';
            console.log(formatLogMessage('debug', `JS redirect detected (${jsRedirectResult.type})${pendingTarget} but not yet executed, waiting...`));
          }
