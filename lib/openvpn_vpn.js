...
//
// '--config' is the one that makes the rest necessary: it pulls in another
// config file, which can itself carry `script-security 2` plus `up`/`down` and
// so reinstate everything the other entries block. '--iproute' names the
// command openvpn shells out to for route setup, and is a direct root-exec
// vector. The remainder are script hooks.
//
// buildArgs drops a blocked flag's value along with the flag. That relies on
// one property, which holds for every entry: a non-'--' token following one of
// these is that flag's value. Note it is NOT true that every entry REQUIRES a
// parameter — openvpn documents '--auth-user-pass [up]' and '--askpass [file]'
// as optional — but a bare token never legitimately follows a parameterless
// flag either, so the heuristic is safe in both shapes.
// '--up-restart' is deliberately absent because it is inert once '--up' and
// '--down' are blocked (it only controls whether they re-run on restart), NOT
// because of anything to do with parameters.
//
// '--writepid' is here for a different reason than the rest: it is not code
// execution but a root-controlled file create/clobber. Verified on openvpn
// 2.6.19, duplicate handling differs per option — '--writepid' is LAST-wins, so
// an extra_args copy overrode the one buildArgs sets and pointed the root-written
// pid file at any path (also leaking it past our cleanup, which only looks in
// TEMP_DIR). Nothing reads that file — child.pid drives findTunDevice,
// waitForProcessExit and the activeConnections entry — so blocking it costs
// nothing. '--log'/'--log-append' are deliberately NOT blocked: those are
// FIRST-wins and buildArgs emits ours first, so a config copy is already inert.
// Poll cadence for waitForProcessExit; was a shelled-out `sleep 0.2`.
...
const BLOCKED_EXTRA_ARGS = [
  ...
  '--config', '--iproute', '--client-connect', '--client-disconnect',
  '--learn-address', '--tls-crypt-v2-verify', '--route-pre-down', '--askpass',
  '--writepid'
];
// The single rule for a connection name, shared by validateOvpnConfig and the
// `pkill -f` guard in startConnection/stopConnection. It permits '.' because
// resolveConnectionName also derives a name from path.basename(config,
// '.ovpn'), so 'us-east.prod.ovpn' yields 'us-east.prod'. Traversal stays
// impossible: no path separator is permitted, and every path.join site appends
// an extension, so the joined component is never a bare '..'.
const CONN_NAME_RE = /^[a-zA-Z0-9_.-]{1,64}$/;

/**
 * Build the `pkill -f` pattern for a connection, with the name regex-escaped.
 * CONN_NAME_RE permits '.', which is a regex metacharacter, so an unescaped
 * 'us.east' would also match 'usXeast' and could TERM an unrelated openvpn
 * process. Escaping here is what lets one charset serve both the path-safety
 * check and this pattern instead of needing two disagreeing rules.
 * @param {string} connectionName - Already CONN_NAME_RE-validated name
 * @returns {string} Pattern safe to pass to `pkill -f`
 */
function pkillPattern(connectionName) {
  const escaped = connectionName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return `openvpn.*${escaped}`;
}

/**
 * True if an extra_args entry names a BLOCKED_EXTRA_ARGS directive, in either
 * the `--flag value` or `--flag=value` form. Shared by validateOvpnConfig
 * (which hard-fails the config) and buildArgs (defense in depth).
 * @param {*} arg - One entry from vpnConfig.extra_args
 * @returns {boolean} true when the entry is a disallowed directive
 */
function isBlockedExtraArg(arg) {
  const s = String(arg);
  const flagPart = s.split('=')[0];
  return BLOCKED_EXTRA_ARGS.some(b => flagPart === b || s.startsWith(b + '='));
}

/**
 * Render an extra_args entry for a log/error message without ever throwing.
 * JSON.stringify returns the VALUE undefined for an undefined or function
 * entry (which join() then swallows, naming nothing) and throws outright on a
 * BigInt — inside the very message meant to report the bad entry.
 * @param {*} a - One entry from vpnConfig.extra_args
 * @returns {string} A printable representation, quoted where JSON allows
 */
function describeExtraArg(a) {
  try {
...
function buildArgs(configPath, vpnConfig, connectionName, logPath) {
  ...
  // directives). validateOvpnConfig already hard-fails a config that names one,
  // so this is defense in depth for the exported low-level startConnection path.
  // A blocked flag's VALUE is dropped along with it: leaving the bare value
  // behind does NOT pass through harmlessly — openvpn aborts with "Unrecognized
  // option or missing or extra parameter(s)" attributed to the PRECEDING option,
  // i.e. a dead tunnel and a misleading error instead of a sanitized command.
  if (vpnConfig.extra_args && Array.isArray(vpnConfig.extra_args)) {
    const safeArgs = [];
    const droppedArgs = [];
    for (let i = 0; i < vpnConfig.extra_args.length; i++) {
      const arg = vpnConfig.extra_args[i];
      if (isBlockedExtraArg(arg)) {
        droppedArgs.push(arg);
        // In the space-separated form the next token is the flag's value, so
        // drop it too — unless it looks like another flag, meaning this one was
        // given bare (see BLOCKED_EXTRA_ARGS on why that shape is safe too).
        // Test for '--' specifically: openvpn options always use a double dash,
        // so a lone '-' or a negative number is a VALUE, and treating it as a
        // flag left exactly the orphan this loop exists to prevent.
        if (!String(arg).includes('=')) {
          const next = vpnConfig.extra_args[i + 1];
          if (next !== undefined && !String(next).startsWith('--')) {
            droppedArgs.push(next);
            i++;
          }
        }
      } else {
    ...
    if (droppedArgs.length > 0) {
      console.log(formatLogMessage('warn', `${OPENVPN_TAG} extra_args: dropped disallowed flags: ${droppedArgs.map(describeExtraArg).join(', ')}`));
    }
    args.push(...safeArgs);
  }

  // WSL2-specific: force tun device type if needed.
  // Normalize to an array first, mirroring the Array.isArray gate the
  // extra_args loop above uses. `?.some` throws "not a function" whenever
  // extra_args is a non-array (a bare string is the easy mistake), and
  // String(a) covers a non-string ELEMENT (e.g. a numeric 1400) reaching
  // .includes. This is the only branch that touches a raw extra_args value,
  // and both shapes now merely fail validation rather than throwing here.
  const extraArgsList = Array.isArray(vpnConfig.extra_args) ? vpnConfig.extra_args : [];
  if (isWSL() && !extraArgsList.some(a => String(a).includes('dev-type'))) {
...
async function startConnection(configPath, vpnConfig, forceDebug = false) {
  ...
  // spawnSync with an arg array (no shell) blocks shell injection. Regex
  // metacharacters in the name are handled by pkillPattern's escaping, not by
  // the charset — CONN_NAME_RE deliberately permits '.'. The check here is the
  // path-safety gate: a name that fails it should not be used to target
  // processes at all, so skip the pkill rather than guess.
  const nameIsSafe = CONN_NAME_RE.test(connectionName);
  if (!nameIsSafe) {
  ...
  const pkillRes = nameIsSafe
    ? spawnSync('sudo', ['pkill', '-TERM', '-f', pkillPattern(connectionName)], { encoding: 'utf8', timeout: 3000 })
    : { status: -1 };
...
function stopConnection(connectionName, forceDebug = false) {
  ...
  try {
    ...
    // array blocks shell injection; regex metacharacters are handled by
    // pkillPattern's escaping, not by the charset. The check below is the
    // path-safety gate — same reasoning as startConnection.
    if (!CONN_NAME_RE.test(connectionName)) {
      ...
      spawnSync('sudo', ['pkill', '-TERM', '-f', pkillPattern(connectionName)], {
        encoding: 'utf8', timeout: 3000
      });

      const killed = waitForProcessExit(info.pid, 5000);
      if (!killed) {
        spawnSync('sudo', ['pkill', '-9', '-f', pkillPattern(connectionName)], {
          encoding: 'utf8', timeout: 3000
        });
      }
    }
 ...
 *
 * Deliberately synchronous: the callers are teardown paths, and an await here
 * would put the VPN shutdown after a yield, which is exactly where puppeteer's
 * synchronous SIGINT handler pre-empts our async cleanup and leaves the tunnel
 * (and a root openvpn) running.
 *
 * Only ESRCH means the process is gone. The previous version wrapped both the
 * probe and the wait in one try with a bare `catch { return true }`, which read
 * ANY throw as "exited" -- two ways that lied:
 *   - EPERM, not ESRCH, is what process.kill(pid, 0) throws when the target
 *     exists but we may not signal it. openvpn is started under sudo, so it is
 *     root-owned while nwss is not: the very FIRST probe threw EPERM and this
 *     returned true immediately, so stopConnection skipped its `pkill -9`
 *     escalation and activeConnections.delete() forgot a still-live root daemon.
 *   - the wait itself throwing (execSync('sleep 0.2') hitting its own 1s
 *     timeout, or `sleep` missing from PATH) was also reported as "exited".
 * The wait is now outside the try and uses Atomics.wait, so it blocks the same
 * way without spawning a shell per 200ms poll.
 *
 * @param {number} pid - Process ID
 * @param {number} timeout - Max wait time in ms
 * @returns {boolean} True only if the process was observed to be gone
 */
function waitForProcessExit(pid, timeout) {
  const start = Date.now();
  // One 4-byte SharedArrayBuffer reused for every wait in this loop.
  const sleeper = new Int32Array(new SharedArrayBuffer(4));
  while (Date.now() - start < timeout) {
    try {
      // Signal 0 tests only for existence + permission; it sends nothing.
      process.kill(pid, 0);
    } catch (probeErr) {
      if (probeErr.code === 'ESRCH') return true; // no such process: genuinely gone
      // EPERM (root-owned, as openvpn is) or anything unexpected: the process
      // may well still be running, so keep waiting and let the caller escalate
      // on timeout rather than claiming a kill we never observed.
    }
    // Outside the try on purpose: a failure to wait says nothing about the pid.
    Atomics.wait(sleeper, 0, 0, POLL_INTERVAL_MS);
  }
  return false;
}
...
function normalizeOvpnConfig(ovpnConfig) {
  ...
  // Verbosity flows into `--verb <n>`. openvpn is spawned with an argv array
  // and no shell, so a crafted value cannot smuggle a second token onto the
  // command line — it would arrive as one malformed argument. Clamping to 1-6
  // is belt-and-braces: it stops openvpn aborting on an out-of-range --verb.
...
function validateOvpnConfig(ovpnConfig) {
  ...
  // Uses the shared CONN_NAME_RE. This was previously a separate, stricter
  // literal (no '.', max 32) than the pkill guard's, so the two disagreed and
  // the looser one could never fire for a config-provided name — while names
  // derived from a config FILENAME (which may contain dots) were checked only
  // by the looser one. One rule now covers both; see CONN_NAME_RE on why '.'
  // is permitted and why that is still traversal-safe.
  // This DELIBERATELY no longer mirrors wireguard_vpn.js F1
  // (/^[a-zA-Z0-9_-]{1,15}$/): a WireGuard interface name is a kernel netdev
  // name, bounded by IFNAMSIZ and unable to contain dots, whereas an openvpn
  // connection name is only ever a filename stem. Don't "fix" the divergence.
  if (ovpnConfig.name !== undefined && ovpnConfig.name !== null) {
    if (typeof ovpnConfig.name !== 'string' || !CONN_NAME_RE.test(ovpnConfig.name)) {
      result.isValid = false;
      result.errors.push(
        `Invalid 'name' value ${JSON.stringify(ovpnConfig.name)}: ` +
        `must match ${CONN_NAME_RE} (path-safe chars, max 64)`
      );
    }
  }
  ...
  // extra_args: hard-fail on directives that can run code as root. buildArgs
  // filters them as well, but failing here gives the user a named error rather
  // than a tunnel that dies on an obscure openvpn options error.
  if (ovpnConfig.extra_args !== undefined && ovpnConfig.extra_args !== null) {
    if (!Array.isArray(ovpnConfig.extra_args)) {
      // buildArgs gates on Array.isArray, so a non-array never reaches spawn
      // and cannot inject anything — it is simply ignored. Warn rather than
      // fail: rejecting would break a config that connected fine before (just
      // without these args), and there is no security reason to.
      result.warnings.push("'extra_args' is not an array and will be ignored");
    } else {
      // A non-string entry reaches String()-free `.includes()` in buildArgs'
      // WSL branch and throws a TypeError out of connectForSite instead of
      // returning the {success:false} the caller expects. Catch it here, where
      // it reads as the config error it is.
      const nonStringArgs = ovpnConfig.extra_args.filter(a => typeof a !== 'string');
      if (nonStringArgs.length > 0) {
        result.isValid = false;
        result.errors.push(
          `extra_args entries must be strings; got ${nonStringArgs.map(describeExtraArg).join(', ')}`
        );
      }

      const blockedArgs = ovpnConfig.extra_args.filter(isBlockedExtraArg);
      if (blockedArgs.length > 0) {
        result.isValid = false;
        result.errors.push(
          `extra_args contains disallowed flag(s): ${blockedArgs.map(describeExtraArg).join(', ')}. ` +
          `These can execute code as root. Use the 'auth_file' / 'username' / 'password' options for credentials.`
        );
      }
    }
  }
...
async function connectForSite(siteConfig, forceDebug = false) {
  ...
  // Validation WARNINGS were previously read only by --validate-config, so on a
  // real scan a config whose extra_args is silently ignored looked identical to
  // a healthy one. Logged BEFORE the isValid return so a failing config still
  // shows its warnings. Gated on forceDebug: unconditional logging would repeat
  // "requires root privileges" on every single connect.
  if (forceDebug && validation.warnings.length > 0) {
    validation.warnings.forEach(w =>
      console.log(formatLogMessage('debug', `${OPENVPN_TAG} config warning: ${w}`))
    );
  }
