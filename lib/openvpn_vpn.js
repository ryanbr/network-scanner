  ...
  if (vpnConfig.extra_args && Array.isArray(vpnConfig.extra_args)) {
    ...
    for (let i = 0; i < vpnConfig.extra_args.length; i++) {
      ...
      if (isBlockedExtraArg(arg)) {
        ...
        // Every blocked directive takes a parameter. In the space-separated
        // form the next token is that parameter, so drop it too — unless it
        // looks like another flag, meaning this one was given bare. Test for
        // '--' specifically: openvpn options always use a double dash, so a
        // lone '-' or a negative number is a VALUE, and treating it as a flag
        // left exactly the orphan this loop exists to prevent.
