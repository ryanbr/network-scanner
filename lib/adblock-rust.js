...
// Anchored, so it only inspects the first few characters of each URL. blob: is
// included even though a blob URL embeds an origin (blob:https://site/uuid): the
// engine cannot parse it either, and lib/adblock.js reports no_match for it too.
...
function parseAdblockRules(filePathOrArray, options = {}) {
  ...
  if (!engine) {
    ...
    for (let i = 0; i < buffers.length; i++) {
      ...
      for (let j = 0; j < lines.length; j++) {
        ...
        // $popup rules are dropped by adblock-rs itself -- verified: such a rule
        // matches nothing there, for any request type. Counting them would
        // overstate what this engine can act on, and would put the banner back out
        // of step with lib/adblock.js, which now skips them at parse time. The
        // engine still RECEIVES every line via addFilters below; only the count
        // changes.
        const optsSegment = trimmed.split('$')[1];
        if (optsSegment) {
          const optionNames = optsSegment.split(',')
  ...
  return {
    shouldBlock(url, sourceUrl, resourceType) {
      // Schemes with no host. A network filter matches on hostname or URL
      // pattern, so none of these can ever match one -- but the native engine
      // does not shrug, it THROWS ("hostname parsing failed"), which cost a
      // warning per request: measured 36 in a four-site scan, plus a bogus
      // "Engine errors" total at the end. Answering here also matches what
      // lib/adblock.js returns for the same URLs ('no_match'), so the two
      // engines no longer disagree on the reason string.
      //
      // Deliberately before the result cache: these are trivial answers and do
      // not belong in it, and counting them as cache misses skewed the hit rate.
