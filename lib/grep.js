...
async function grepContent(content, searchPatterns, options = {}) {
  ...
  // `if (!pattern || ...) continue` inside the loop.
  const validPatterns = Array.isArray(searchPatterns)
    ? searchPatterns.filter(p => p && p.trim().length > 0)
    : [];
  ...
  for (const pattern of validPatterns) {
    ...
    // Also accept truncated output (the output cap kicked in but we
    // collected enough to confirm matches and slice to maxMatches).
