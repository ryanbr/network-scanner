  ...
  try {
    ...
    for (const pattern of searchPatterns) {
      ...
      try {
        ...
        console.warn(`[warn][grep] Pattern "${pattern}" failed: ${grepErr.message}`);
      }
    }
    ...
    if (tempFile) {
      try {
        ...
        console.warn(`[warn][grep] Failed to cleanup temp file ${tempFile}: ${cleanupErr.message}`);
      }
    }
  }
}
...
function createGrepHandler(config) {
  ...
  return async function grepHandler(requestUrl) {
    ...
    if (isFirstParty && siteConfig.firstParty === false) {
      if (forceDebug) {
        console.log(`[debug][grep] Skipping first-party request (firstParty=false): ${requestUrl}`);
      }
      return;
    }
    ...
    if (!isFirstParty && siteConfig.thirdParty === false) {
      if (forceDebug) {
        console.log(`[debug][grep] Skipping third-party request (thirdParty=false): ${requestUrl}`);
      }
      return;
    }
    ...
    try {
      if (forceDebug) {
        console.log(`[debug][grep] Downloading and searching content from: ${requestUrl}`);
      }
      ...
      if (!hasSearchString) {
        ...
        if (siteConfig.verbose === 1) {
          ...
          console.log(`[match][${simplifiedUrl}] ${requestUrl} (${partyType}, grep) matched regex`);
        }
      ...
      if (result.found) {
        ...
        if (siteConfig.verbose === 1) {
          ...
          console.log(`[match][${simplifiedUrl}] ${requestUrl} (${partyType}, grep) contains pattern: "${result.matchedPattern}" (${matchCount} matches)`);
        }
        ...
        const partyType = isFirstParty ? 'first-party' : 'third-party';
        console.log(`[debug][grep] ${requestUrl} (${partyType}) matched regex but no patterns found`);
      }
      
    } catch (err) {
      if (forceDebug) {
        console.log(`[debug][grep] Failed to download/grep content for ${requestUrl}: ${err.message}`);
      }
    }
  };
}
