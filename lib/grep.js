...
async function downloadAndGrep(url, searchPatterns, userAgent = '', grepOptions = {}, timeout = GREP_DEFAULTS.TIMEOUT_SECONDS) {
  ...
  // --proto is the load-bearing half (a `file://` initial URL is otherwise read
  // straight off disk); --proto-redir mostly restates curl's defaults and adds
  // ftp/ftps. Mirrors lib/curl.js.
  ...
  // '--' end-of-options marker before the URL, mirroring lib/curl.js: without
  // it a URL that begins with '-' would be reinterpreted by curl as a flag.
  curlArgs.push('--', url);
...
function createGrepHandler(config) {
  ...
  return async function grepHandler(requestUrl) {
    ...
    // Previously this ran AFTER URL parses and a detected-domain lookup,
