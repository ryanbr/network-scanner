...
function shouldProcessUrl(url, forceDebug = false) {
  if (!url || typeof url !== 'string') {
    if (forceDebug) console.log(`${FLOWPROXY_TAG}[url-validation] Skipping invalid URL: ${url}`);
    return false;
  }
  ...
  for (const pattern of SKIP_PATTERNS) {
    if (urlLower.startsWith(pattern)) {
      if (forceDebug) {
        console.log(`${FLOWPROXY_TAG}[url-validation] Skipping ${pattern} URL: ${url.substring(0, 100)}${url.length > 100 ? '...' : ''}`);
      }
      return false;
    }
  }
  ...
  if (!urlLower.startsWith('http://') && !urlLower.startsWith('https://')) {
    if (forceDebug) {
      console.log(`${FLOWPROXY_TAG}[url-validation] Skipping non-HTTP(S) URL: ${url.substring(0, 100)}${url.length > 100 ? '...' : ''}`);
    }
    return false;
  }
