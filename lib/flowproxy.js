...
async function safePageEvaluate(page, func, timeout = TIMEOUTS.PAGE_EVALUATION_SAFE) {
  ...
  try {
    ...
    // Return full default-false shape (matches analyzeFlowProxyProtection's
    // outer catch) so downstream `.hasProtectionPage` etc. read as `false`
    // instead of `undefined` — keeps debug logs honest and conditional
    // branches in handleFlowProxyProtection deterministic.
    return {
