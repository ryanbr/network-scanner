...
async function analyzeFlowProxyProtection(page) {
  try {
    ...
    return await safePageEvaluate(page, () => {
      ...
      const hasSpecificSignal = hasFlowProxyDomain ||
      ...
      // PRIMARY DETECTION: at least one vendor-specific signal must be
      // present. Tightened from the old `any-of-four` (which let a single
      // "Please wait" in any page title fire the full 30s handling
      // pipeline) to require an unambiguous FlowProxy/Aurologic marker.
      const isFlowProxyDetected = hasSpecificSignal;
  ...
  try {
    ...
    if (detectionInfo.isFlowProxyDetected) {
      ...
      if (forceDebug) {
        ...
        console.log(formatLogMessage('debug', `${FLOWPROXY_TAG}   Specific signals: domain=${detectionInfo.hasFlowProxyDomain} elements=${detectionInfo.hasFlowProxyElements} scripts=${detectionInfo.hasFlowProxyScripts} brandText=${detectionInfo.hasFlowProxyBrandText}`));
