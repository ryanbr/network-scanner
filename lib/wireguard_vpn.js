...
function interfaceUp(configPath, interfaceName, forceDebug = false) {
  ...
  try {
    ...
    let upRes = spawnSync('sudo', ['wg-quick', 'up', configPath], {
      encoding: 'utf8',
      timeout: 15000
    });
    if (upRes.error) throw upRes.error;
    if (upRes.status !== 0) {
      const stderr = (upRes.stderr || '').trim();
      // F2: Detect "already exists" — interface persisted in the kernel
      // from a prior failed teardown (interfaceDown's catch deletes the
      // tracking entry even when wg-quick down fails, so activeInterfaces
      // can be out-of-sync with the kernel). Self-heal by deleting the
      // stale link directly and retrying once. Avoids manual recovery
      // when wg-quick down had a transient failure and left the kernel
      // interface alive.
      if (stderr.includes('already exists') || stderr.includes('File exists')) {
        if (forceDebug) {
          console.log(formatLogMessage('debug', `${VPN_TAG} ${interfaceName} exists in kernel from prior run; cleaning up and retrying`));
        }
        // ip link delete is more aggressive than wg-quick down — works
        // even when the corresponding wg-quick config is missing/stale.
        spawnSync('sudo', ['ip', 'link', 'delete', interfaceName], { encoding: 'utf8', timeout: 5000 });
        upRes = spawnSync('sudo', ['wg-quick', 'up', configPath], { encoding: 'utf8', timeout: 15000 });
        if (upRes.error) throw upRes.error;
        if (upRes.status !== 0) {
          throw new Error((upRes.stderr || '').trim() || `wg-quick up retry exited with status ${upRes.status}`);
        }
      } else {
        throw new Error(stderr || `wg-quick up exited with status ${upRes.status}`);
      }
    }
