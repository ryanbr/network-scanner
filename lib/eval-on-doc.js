 ...
 *
 * It counts DOCUMENT loads, via 'domcontentloaded'. Puppeteer's 'framenavigated'
 * looked like the natural signal and is wrong: it also fires for same-document
 * navigation. Measured -- a page doing `history.replaceState` to its own url six
 * times, never reloading once, was reported as a reload loop; and an SPA churning
 * history fired it 13 times for a single document. A history rewrite is not a
 * load, and saying it is produces a warning that is simply false.
 *
 ...
 *   `loads()` is the number of document loads seen, `loopedUrl()` the url that
...
function watchForReloadLoop(page, {
  ...
  const onDocumentLoaded = () => {
    try {
      ...
      // page.url() throws once the page is detaching. A missed count is harmless
      // when the only consequence is a log line.
    }
  };
  ...
  return {
    ...
    stop() {
      if (stopped) return;
      stopped = true;
      try { page.off('domcontentloaded', onDocumentLoaded); } catch (_) { /* page gone */ }
    }
  };
}
