...
function createMatcher(rules, options = {}) {
  ...
  // Two-level result cache: url → Map(sourceUrl+type → result)
  // Avoids string concatenation GC pressure on every shouldBlock() call
  const resultCache = new Map();
  const RESULT_CACHE_MAX = 16000;
  let resultCacheSize = 0;

  function resultCacheGet(url, sourceUrl, resourceType) {
    const inner = resultCache.get(url);
    if (!inner) return undefined;
    return inner.get(sourceUrl + resourceType);
  }

  function resultCacheSet(url, sourceUrl, resourceType, result) {
    if (resultCacheSize >= RESULT_CACHE_MAX) {
      resultCache.clear();
      resultCacheSize = 0;
    }
    let inner = resultCache.get(url);
    if (!inner) {
      inner = new Map();
      resultCache.set(url, inner);
    }
    const key = sourceUrl + resourceType;
    if (!inner.has(key)) resultCacheSize++;
