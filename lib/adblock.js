...
// One definition of "this url has no host", shared with lib/adblock-rust.js so the
// two engines cannot drift apart on it.
...
function createPopupSignalMatcher(filePathOrArray) {
  ...
  return {
    ...
    match(url, sourceUrl) {
      ...
      // Same reason the blocking path skips these: a hostless URI carries its own
      // content, so a path rule matches a substring inside it -- measured,
      // `data:text/html,<a href="/pop-target.html">` reported as a known popunder
      // endpoint. nwss cannot reach this today (evaluatePopupUrl returns early when
      // a url yields no root domain), but a function that answers a question
      // wrongly for its own inputs is a trap for the next caller.
      if (isHostlessUrl(url)) return null;
