  ...
  try {
    return content.replace(
      ...
      (match) => {
        ...
        if (num >= 0 && num <= 0x10FFFF) return String.fromCharCode(num);
...
function createResponseHandler(config) {
  ...
  // Was parsed once per response (could be 100+ per page).
  let currentUrlHostname = '';
  try { currentUrlHostname = new URL(currentUrl).hostname; } catch (_) {}
  ...
  return async function responseHandler(response) {
    ...
    const respDomain = perSiteSubDomains ? respHostname : getRootDomain(respUrl);
    ...
    // First-party / third-party gate. The main handler already applied
    // this filter to requests, but we re-check here because responses
    // are a separate observation channel (content matters, not URL).
    const isFirstParty = currentUrlHostname === respHostname;
    if (isFirstParty && siteConfig.firstParty === false) {
      if (forceDebug) {
