...
function createCurlHandler(config) {
  ...
  return async function curlHandler(requestUrl) {
    ...
    if (typeof config.isDomainAlreadyDetected === 'function' && config.isDomainAlreadyDetected(reqDomain)) {
      if (forceDebug) {
        console.log(formatLogMessage('debug', `[curl] Skipping already detected domain: ${reqDomain}`));
      }
      return;
    }
    ...
    if (isFirstParty && siteConfig.firstParty === false) {
      if (forceDebug) {
        console.log(formatLogMessage('debug', `[curl] Skipping first-party request (firstParty=false): ${requestUrl}`));
      }
      return;
    }
    ...
    if (!isFirstParty && siteConfig.thirdParty === false) {
      if (forceDebug) {
        console.log(formatLogMessage('debug', `[curl] Skipping third-party request (thirdParty=false): ${requestUrl}`));
      }
      return;
    }
    ...
    try {
      if (forceDebug) {
        console.log(formatLogMessage('debug', `[curl] Downloading content from: ${requestUrl}`));
      }
      ...
      if (found) {
        ...
        const partyType = isFirstParty ? 'first-party' : 'third-party';
        console.log(formatLogMessage('debug', `[curl] ${requestUrl} (${partyType}) matched regex but no searchstring found`));
        if (error) {
          console.log(formatLogMessage('debug', `[curl] Search error: ${error}`));
        }
      }
      
    } catch (err) {
      if (forceDebug) {
        console.log(formatLogMessage('debug', `[curl] Failed to download content for ${requestUrl}: ${err.message}`));
      }
    }
  };
}
