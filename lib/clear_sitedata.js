...
async function clearSiteDataViaCDP(page, currentUrl, forceDebug, quickMode = false) {
  ...
  try {
    ...
    // localStorage + sessionStorage — now cleared in quick mode too. These
    // are where AdsCore-family popunder networks track their per-session cap
    // (aclibSubKey-popunder and similar localStorage keys; see commit
    // 11e1f49 + the obfuscated-script review). Previously the quick-mode
    // skip left localStorage intact across reloads, capping popunder
    // discovery at ~1 capture per scan even with reload: N + clear_sitedata
    // true. Single CDP call per storage type, sub-millisecond each — the
    // perf reason to skip them in quick mode no longer holds. Heavier types
    // (IndexedDB, WebSQL, service workers) still gate on !quickMode below.
    const originScopedQuickStorage = [
      { type: 'local_storage', name: 'localStorage' },
      { type: 'session_storage', name: 'sessionStorage' }
    ];
    for (const storage of originScopedQuickStorage) {
      try {
        ...
        if (forceDebug) {
          console.log(formatLogMessage('debug', `${CLEAR_SITEDATA_TAG} ${storage.name} clear failed: ${storageErr.message}`));
        }
      }
    }
