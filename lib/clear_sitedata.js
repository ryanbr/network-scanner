...
const CLEAR_SITEDATA_ENHANCED_TAG = messageColors.processing('[clear_sitedata_enhanced]');
  ...
  try {
    ...
    for (const op of enhancedOperations) {
      try {
        ...
        if (forceDebug) {
          console.log(formatLogMessage('debug', `${CLEAR_SITEDATA_ENHANCED_TAG} ${op.name} failed: ${opErr.message}`));
        }
    ...
    try {
      ...
      if (forceDebug) {
        console.log(formatLogMessage('debug', `${CLEAR_SITEDATA_ENHANCED_TAG} DOM cleanup failed: ${domErr.message}`));
      }
    }
    
    if (forceDebug && completedOperations.length > 0) {
      console.log(formatLogMessage('debug', `${CLEAR_SITEDATA_ENHANCED_TAG} Cleared: ${completedOperations.join(', ')}`));
    }
    ...
    if (forceDebug) {
      console.log(formatLogMessage('debug', `${CLEAR_SITEDATA_ENHANCED_TAG} Failed: ${enhancedErr.message}`));
    }
