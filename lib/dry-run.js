  ...
  try {
    ...
    matchedItems.forEach((item, index) => {
      try {
        ...
        // Show searchstring "not found" result when a match was queued
        // for content checking but no searchstring fired. The positive
        // "Searchstring Match" branch was removed — addDryRunSearchString
        // never had any callers, so dryRunSearchString was always empty
        // and item.searchStringMatch was always null. Keep the negative
        // path since item.searchStringChecked comes from the incoming
        // match.needsSearchStringCheck flag (still set upstream).
