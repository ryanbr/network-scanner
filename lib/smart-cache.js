  ...
  static clearPersistentCache(options = EMPTY) {
    ...
    if (stats && stats.isDirectory()) {
      ...
      for (const name of entries) {
        if (isOwnFile(name)) removeFile(path.join(cachePath, name), 'file');
      }
      ...
      if (foreign.length === 0) {
        ...
        console.log(formatLogMessage('debug', `${SMART_CACHE_TAG} Kept ${cachePath}: ${foreign.length} file(s) in it are not ours`));
      }
    } else if (stats) {
