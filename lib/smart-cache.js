 ...
 */
    ...
    try {
      ...
      fs.writeFile(tmpFile, jsonStr, { mode: 0o600 }, (writeErr) => {
        if (writeErr) {
          ...
          // Drop the (possibly partial) temp file. The pid suffix makes the
          // name unique per process, so a failed write orphans it forever --
          // the old fixed '.tmp' name was simply overwritten on the next save.
          fs.unlink(tmpFile, () => {});
          this.saveInProgress = false;
          return;
        }
        ...
        fs.rename(tmpFile, cacheFile, (renameErr) => {
          if (renameErr) {
            ...
            // Rename failed: the temp file is complete but stranded. Remove it
            // so pid-suffixed leftovers don't pile up in the cache dir.
            fs.unlink(tmpFile, () => {});
          } else {
  ...
  static clearPersistentCache(options = EMPTY) {
    ...
    // GUARDED, deliberately: this used to be
    // fs.rmSync(cachePath, { recursive: true, force: true }) on a path that
    // comes straight from the user's `cache_path`, which made --clear-cache a
    // recursive delete of any directory they happened to name -- for a cache
    // nwss may never have written (persistence is off by default, and nwss
    // currently hardcodes it off). Nothing else puts files here either: the
    // adblock-rs disk cache lives in os.tmpdir()/nwss-adblock-rs-cache, not
    // in cachePath. So only this module's own files are removed by name, and
    // the directory goes only if that leaves it empty.
    const OWN_FILE = 'smart-cache.json';
    // Temps are OWN_FILE + '.' + pid + '.tmp' (see savePersistentCache); a
    // failed write orphans one, so they are ours to collect.
    const isOwnFile = (name) => name === OWN_FILE || (name.startsWith(OWN_FILE + '.') && name.endsWith('.tmp'));
    // A temp carries the pid that is writing it. Skip one whose process is still
    // alive: that is an in-flight write, and removing it makes the other
    // process's rename fail. Liveness convention is the repo's — only ESRCH
    // means gone; EPERM means alive but owned by another user.
    const TMP_PID_RE = new RegExp('^' + OWN_FILE.replace(/\./g, '\\.') + '\\.(\\d+)\\.tmp$');
    const isLiveTemp = (name) => {
    ...
    const removeFile = (fullPath, label) => {
      try {
        const st = fs.statSync(fullPath);
        fs.unlinkSync(fullPath);
        totalSize += st.size;
        clearedItems++;
        clearedFiles.push({ type: 'file', path: fullPath, size: st.size });
        if (forceDebug) {
          console.log(formatLogMessage('debug', `${SMART_CACHE_TAG} Cleared cache ${label}: ${fullPath}`));
        }
      } catch (err) {
        if (err.code !== 'ENOENT') {
          errors.push({ path: fullPath, error: err.message });
          if (forceDebug) {
            console.log(formatLogMessage('debug', `${SMART_CACHE_TAG} Failed to clear ${fullPath}: ${err.message}`));
          }
        }
      }
    };
    ...
    try {
      ...
      if (statErr.code !== 'ENOENT') {
        errors.push({ path: cachePath, error: statErr.message });
        if (forceDebug) {
          console.log(formatLogMessage('debug', `${SMART_CACHE_TAG} Cannot stat ${cachePath}: ${statErr.message}`));
        }
      }
    }
    ...
    if (stats && stats.isDirectory()) {
      ...
      try {
        ...
        errors.push({ path: cachePath, error: readErr.message });
        if (forceDebug) {
          console.log(formatLogMessage('debug', `${SMART_CACHE_TAG} Cannot read ${cachePath}: ${readErr.message}`));
        }
      }

      const liveTemps = [];
      for (const name of entries) {
        if (!isOwnFile(name)) continue;
        if (isLiveTemp(name)) { liveTemps.push(name); continue; }
        removeFile(path.join(cachePath, name), 'file');
      }

      // Never rmdir through a symlink: rmdir on the link itself fails with
      // ENOTDIR, which surfaced as a spurious error even though the clear had
      // succeeded — and removing someone's symlink is not ours to do.
      let isLink = false;
      try {
        isLink = fs.lstatSync(cachePath).isSymbolicLink();
      } catch (lstatErr) { /* unreadable: treat as not a link, rmdir will decide */ }

      // Remove the directory only when our files were all it held, so a
      // dedicated .cache still disappears while anything else is left alone.
      const foreign = entries.filter(name => !isOwnFile(name));
      if (foreign.length === 0 && liveTemps.length === 0 && !isLink) {
        try {
          fs.rmdirSync(cachePath);
          clearedItems++;
          clearedFiles.push({ type: 'directory', path: cachePath, size: 0 });
          if (forceDebug) {
            console.log(formatLogMessage('debug', `${SMART_CACHE_TAG} Removed empty cache directory: ${cachePath}`));
          }
        } catch (rmErr) {
          // ENOTEMPTY means something appeared under us -- keep the directory.
          if (rmErr.code !== 'ENOENT' && rmErr.code !== 'ENOTEMPTY') {
            errors.push({ path: cachePath, error: rmErr.message });
          }
        }
      } else if (forceDebug) {
        const why = [];
        if (foreign.length > 0) why.push(`${foreign.length} file(s) in it are not ours`);
        if (liveTemps.length > 0) why.push(`${liveTemps.length} temp file(s) still being written (${liveTemps.join(', ')})`);
        if (isLink) why.push('it is a symlink');
        console.log(formatLogMessage('debug', `${SMART_CACHE_TAG} Kept ${cachePath}: ${why.join(', ')}`));
      }
    } else if (stats) {
      // cachePath is a file. It is documented as a directory, so only honour it
      // when it IS our cache file -- never unlink an arbitrary path the user named.
      if (path.basename(cachePath) === OWN_FILE) {
        removeFile(cachePath, 'file');
      } else {
        errors.push({ path: cachePath, error: `not a directory and not ${OWN_FILE} -- refusing to delete` });
        if (!silent) {
          console.warn(messageColors.warn(`Refusing to clear ${cachePath}: not a cache directory`));
        }
      }
    }

    // Orphaned file directly at cachePath's parent level is covered above; a
    // missing directory simply clears nothing.
    const result = {
