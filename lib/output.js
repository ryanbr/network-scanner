...
function formatDomain(domain, options = {}) {
  const { localhost = false, localhostAlt = false, plain = false, adblockRules = false, dnsmasq = false, dnsmasqOld = false, unbound = false, privoxy = false, resourceType = null } = options;
...
function formatRules(matchedDomains, siteConfig = {}, globalOptions = {}) {
  ...
  if (matchedDomains instanceof Map && formatOptions.adblockRules) {
    ...
    // Handle Set format (legacy behavior) or other modes (including privoxy)
    const domainsToProcess = matchedDomains instanceof Set ? matchedDomains : new Set(matchedDomains.keys());
    domainsToProcess.forEach(domain => {
      const formatted = formatDomain(domain, formatOptions);
      if (formatted) {
...
function getFormatDescription(options = {}) {
  const { localhost = false, localhostAlt = false, plain = false, adblockRules = false, dnsmasq = false, dnsmasqOld = false, unbound = false, privoxy = false } = options;
