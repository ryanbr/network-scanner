...
/**
 * Whether a matched key can be emitted as a rule.
 *
 * This replaced a flat `domain.length <= 6` test, which silently dropped real
 * domains from EVERY output format: bit.ly, goo.gl, vk.com, qq.com, adf.ly,
 * t.co, x.com, ok.ru and is.gd all produced no rule at all, with no warning,
 * because each caller just skips a null. The length was also measured on the
 * whole key including any path, so `t.co/ads/` (9 chars) sailed through while
 * the bare host did not — proof the number wasn't protecting anything
 * structural. What the guard is actually for is junk from `output_regex`
 * captures, so the checks are structural instead: a dot, no empty labels, a
 * two-character-minimum TLD, and at least `x.yy`. IP literals are accepted
 * explicitly, since a request to a bare address is a legitimate rule and a
 * numeric last label would otherwise fail the TLD check.
 *
 * @param {string} host - Host portion of the matched key (no path)
 * @returns {string|null} null if emittable, else a short reason for debug logs
 * @private
 */
function hostRejectionReason(host) {
  if (!host) return 'empty host';
  // IPv4 only. A bare IPv6 host would fail the TLD check below, but it is also
  // wrong in most output formats (`local=/::1/`, `0.0.0.0 ::1`), and it cannot
  // reach here from a scan anyway: URL parsing keeps the brackets, and
  // '[2001:db8::1]' has no dot so it is refused exactly as it was before.
  if (net.isIP(host) === 4) return null; // bare IPv4 rules are legitimate
  if (!host.includes('.')) return 'no dot';
  const labels = host.split('.');
  for (let i = 0; i < labels.length; i++) {
    if (labels[i].length === 0) return 'empty label';
  }
  if (labels[labels.length - 1].length < 2) return 'single-character TLD';
  if (host.length < 4) return `shorter than 4 characters (${host.length})`;
  return null;
}
...
function formatDomain(domain, options = {}) {
  const { localhostIP = null, plain = false, adblockRules = false, dnsmasq = false, dnsmasqOld = false, unbound = false, privoxy = false, pihole = false, resourceType = null, forceDebug = false } = options;
  ...
  const rawHost = isPathRule ? domain.slice(0, slash) : domain;
  const pathPart = isPathRule ? domain.slice(slash) : '';

  // A single trailing dot is the legal root-anchored form of the same host, and
  // Chrome preserves it: new URL('http://ads.example.com./t.js').hostname is
  // 'ads.example.com.'. No output format wants it (it would emit ||host.^, which
  // matches nothing), and refusing it would silently drop a real domain — the
  // very failure this guard caused before. So normalise one dot and continue;
  // anything further ('host..') still trips the empty-label check below.
  const host = rawHost.length > 1 && rawHost.endsWith('.') ? rawHost.slice(0, -1) : rawHost;

  // Validate the HOST, not the whole key — a path must not be able to pad a
  // malformed host past the check (or vice versa).
  const rejection = hostRejectionReason(host);
  if (rejection) {
    if (forceDebug) {
      console.log(formatLogMessage('debug', `${OUTPUT_FILTER_TAG} Not emitting ${domain}: ${rejection}`));
    }
    return null;
  }
  ...
  if (pihole) {
    ...
    return isPathRule ? `||${host}${pathPart}${resourceType}` : `||${host}^${resourceType}`;
  } else {
    // Default adblock: ||host^ for a domain, ||host/path/ for a path rule
    // (the path already anchors, so no trailing '^').
    return isPathRule ? `||${host}${pathPart}` : `||${host}^`;
  }
}
