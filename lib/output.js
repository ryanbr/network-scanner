...
function formatDomain(domain, options = {}) {
  const { localhost = false, localhostAlt = false, plain = false } = options;
 ...
 * @param {Set<string>} matchedDomains - Set of matched domains
...
function formatRules(matchedDomains, siteConfig = {}, globalOptions = {}) {
  ...
  // Plain output takes precedence over localhost options when enabled
  const formatOptions = {
...
function handleOutput(results, config = {}) {
  ...
  // Write main output
  const mainSuccess = writeOutput(outputLines, outputFile, silentMode);
  ...
  return {
    ...
    outputLines: outputFile ? null : outputLines // Only return lines if not written to file
  };
}
