...
function formatDomain(domain, options = {}) {
  const { localhost = false, localhostAlt = false, plain = false, adblockRules = false, dnsmasq = false, dnsmasqOld = false, unbound = false, resourceType = null } = options;
...
function formatRules(matchedDomains, siteConfig = {}, globalOptions = {}) {
  ...
  if (matchedDomains instanceof Map && formatOptions.adblockRules) {
    ...
    // Handle Set format (legacy behavior) or dnsmasq mode
...
function getFormatDescription(options = {}) {
  const { localhost = false, localhostAlt = false, plain = false, adblockRules = false, dnsmasq = false, dnsmasqOld = false, unbound = false } = options;
