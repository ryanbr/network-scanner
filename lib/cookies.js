...
function normalizeCookies(rawCookies, targetUrl) {
  ...
  try {
    ...
    if (targetHost) {
      const site = registrableDomain(targetHost);
      defaultDomain = site ? `.${site}` : targetHost;
    }
  } catch (_) {
  ...
  for (const entry of entries) {
    ...
    if (targetHost && !domain.startsWith('.') &&
        domain.toLowerCase() !== targetHost &&
        targetHost.endsWith(`.${domain.toLowerCase()}`)) {
