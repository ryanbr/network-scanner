...
function normalizeCookies(rawCookies, targetUrl) {
  ...
  for (const entry of entries) {
    ...
    //
    // The last condition keeps the suggested fix HONEST: it must name a scope the
    // browser would actually store. Without it, `domain: "com"` on
    // www.domain.com suggested ".com", and `domain: "0.0.1"` on 127.0.0.1
    // suggested ".0.0.1" -- both rejected by Chrome, so the advice would send
    // someone to a cookie that cannot exist. Requiring the domain to sit inside
    // the target host's own registrable domain excludes public suffixes and IP
    // fragments, and still catches every genuinely fixable case (including an
    // intermediate 'b.domain.com' under a.b.domain.com).
