        ...
        safeExecute(() => {
          if (!navigator.connection) {
            ...
            const effectiveType = etRoll < 75 ? '4g'
                                ...
                                : 'slow-2g';
            // wifi 70%, cellular 30%
            const type = ((h >>> 12) % 10) < 7 ? 'wifi' : 'cellular';
            // rtt 50-450ms, in 50ms buckets (real Chrome rounds to
            // nearest 25ms for privacy; bucketing matches that shape)
            const rtt = 50 + (((h >>> 8) % 9) * 50);
            // downlink 2.0-50.0 Mbps, 0.1 precision
            const downlink = Math.round((((h >>> 16) % 481) + 20)) / 10;
