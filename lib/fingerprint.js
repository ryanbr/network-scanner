...
// UA string deliberately hides this (Chrome/149.0.0.0), but the full-version
...
// USER_AGENT_COLLECTIONS major AND this build (149 ships 7827.22).
const CHROME_BUILD = '7827.22';
...
async function applyUserAgentSpoofing(page, siteConfig, forceDebug, currentUrl) {
  ...
  if (ua) {
    ...
    try {
      ...
      await page.evaluateOnNewDocument((userAgent, debugEnabled, gpuConfig, seededCores, chromeBuild) => {
      ...
      }, ua, forceDebug, selectedGpu, realistic.hardwareConcurrency, CHROME_BUILD);
    } catch (stealthErr) {
