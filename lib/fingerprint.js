    ...
    try {
      ...
      await page.evaluateOnNewDocument((userAgent, debugEnabled, gpuConfig, seededCores) => {
        ...
        safeExecute(() => {
          ...
          const jitter = (value) => {
            // Don't jitter 0 values -- those signal 'event never occurred'
            // (e.g. secureConnectionStart is 0 for non-HTTPS). Adding noise
            // would invent a connection that didn't happen, breaking the
            // 0 vs >0 boolean check that some scripts use.
            if (typeof value !== 'number' || value === 0) return value;
            return value + (Math.random() - 0.5); // ±0.5ms
          };
          ...
          const wrapEntry = (entry) => {
            ...
            const proxy = new Proxy(entry, {
              get(target, prop) {
                ...
                if (TIMING_FIELDS.has(prop)) return jitter(value);
                if (typeof value === 'function') {
                  // toJSON is what JSON.stringify uses -- override to apply
                  // jitter to the serialised values too.
                  if (prop === 'toJSON') {
                    return function() {
                      const json = value.call(target);
                      for (const f of TIMING_FIELDS) {
                        if (f in json) json[f] = jitter(json[f]);
                      }
                      return json;
                    };
                  }
            ...
            wrappedEntries.set(entry, proxy);
            return proxy;
          };
