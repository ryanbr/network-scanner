...
async function performRealtimeWindowCleanup(browserInstance, threshold = REALTIME_CLEANUP_THRESHOLD, forceDebug, totalDelay = 19000) {
  try {
    ...
    // (Removed: a block that walked browserContexts() looking for "popup
    // contexts" to close. It was dead three times over -- BrowserContext.isIncognito,
    // which guarded it, does not exist in the supported puppeteer (the name is absent
    // from the whole package at 25.12); nothing in nwss.js or lib/ ever creates a
    // non-default context; and popups share their opener's context anyway, measured --
    // window.open leaves browserContexts() at 1 with the popup in the default context.
    // Do not restore it: the body closed EVERY page in a matching context with no
    // isPageSafeToClose and no isProcessing check, so a working guard would have been
    // more dangerous than a broken one. Popup pages are handled by the normal
    // allPagesAfterDelay sweep below, which does make those checks.)
...
async function monitorBrowserHealth(browserInstance, context = {}, options = {}) {
  ...
  const result = {
    shouldRestart: false,
    shouldContinue: true,
    // Machine-readable severity for callers to branch on. `reason` is prose for
    // humans and must NOT be pattern-matched: nwss used to decide with
    // reason.includes('Critical') || reason.includes('disconnected'), which acted
    // on 2 of the 9 verdicts emitted below. It missed "Browser connectivity lost"
    // (the browser is gone) entirely, and missed `Browser health: critical` purely
    // because assessment.overall is lowercase while the pattern had a capital C --
    // so of the two states that code named, only 'disconnected' ever worked.
    //   'critical'  - the browser cannot do its job; restart now
    //   'degraded'  - worth reporting, not worth a restart on its own
    //   'scheduled' - the URL-count interval, which the caller's batch logic owns
    //   'none'      - healthy
    severity: 'none',
    reason: null,
    assessment: null
  ...
  try {
    ...
    if (assessment.browser.criticalError) {
      ...
      result.shouldRestart = true;
        // 'unhealthy' means the basic probe failed (no process, or newPage did
        // not work), 'critical' a protocol error, 'disconnected' a dead
        // transport -- all three mean the browser cannot serve a scan.
        // 'degraded' reaches here only via a restart-worthy recommendation
        // (high memory), which is reported but is NOT being promoted to a
        // restart by this change.
        result.severity = ['unhealthy', 'critical', 'disconnected'].includes(assessment.overall)
          ? 'critical'
          : 'degraded';
    ...
    if (!silentMode && result.shouldRestart) {
      ...
      // A 'degraded' verdict is reported and NOT acted on (see severity above), so it
      // must not announce a restart. Measured with 46 real pages open: this line said
      // "Browser restart needed" and nothing restarted -- the same mismatch the
      // severity field exists to remove. Fixing the decision and leaving the sentence
      // would only have moved the lie.
      const degraded = result.severity === 'degraded';
      const headline = degraded
        ? messageColors.warn('Browser health degraded')
        : messageColors.fileOp('Browser restart needed');
      console.log(`\n${headline} before site${progress}: ${result.reason}${degraded ? ' (continuing without a restart)' : ''}`);
    }
    ...
    result.shouldRestart = true;
    // Degraded, not critical: a monitor failure is usually transient, and the
    // connectivity branch above catches a genuinely dead browser next pass.
    result.severity = 'degraded';
