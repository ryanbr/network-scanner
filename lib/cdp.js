...
async function createCDPSession(page, currentUrl, options = {}) {
  ...
  if (enableCDP) {
    console.log(formatLogMessage('debug', `[cdp] Global CDP enabled by --cdp flag for ${currentHostname}`));
  } else if (siteSpecificCDP === true) {
    console.log(formatLogMessage('debug', `[cdp] Site-specific CDP enabled for ${currentHostname} (via cdp: true or cdp_specific domain match)`));
  }
  ...
  try {
    ...
    if (cdpSession) {
      try { await cdpSession.detach(); }
        ...
        console.log(formatLogMessage('debug', `[cdp] Partial-session detach failed for ${currentUrl}: ${partialDetachErr.message}`));
      }
      cdpSession = null;
    }
    ...
    console.warn(formatLogMessage('warn', `[cdp] Failed to attach CDP session for ${urlContext}: ${cdpErr.message}`));
