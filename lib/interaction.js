...
function generateRandomCoordinates(maxX = DEFAULT_VIEWPORT.WIDTH, maxY = DEFAULT_VIEWPORT.HEIGHT, options = {}) {
  ...
  // Usable span once the margins are removed. Hoisted here for two reasons:
  // it was recomputed inline six times below, and the <= 0 guard now covers
  // EVERY branch. On a viewport smaller than its own margins rangeX/rangeY go
  // negative and Math.random() * negative yields negative / out-of-bounds
  // coords — preferEdges and avoidCenter used the same math as the standard
  // branch and were still exposed while only the latter was guarded.
  ...
  if (preferEdges) {
    // Clamp the edge zone to the margin-free span. EDGE_ZONE_SIZE is a fixed
    // 200px, so on a narrow viewport the right/bottom formulas
    // (max - EDGE_ZONE_SIZE - margin) went negative and the left/top ones
    // overshot into the opposite margin — e.g. 101x101 produced y = -124.
    // min() is a no-op on any viewport with a >=200px usable span, so normal
    // resolutions are unaffected.
    const edgeZoneX = Math.min(COORDINATE_MARGINS.EDGE_ZONE_SIZE, rangeX);
    const edgeZoneY = Math.min(COORDINATE_MARGINS.EDGE_ZONE_SIZE, rangeY);

    // Prefer coordinates near edges for more realistic behavior
    const edge = Math.random();
    if (edge < PROBABILITIES.EDGE_PREFERENCE.LEFT) {
      // Left edge
      x = Math.floor(Math.random() * edgeZoneX) + marginX;
      y = Math.floor(Math.random() * rangeY) + marginY;
    } else if (edge < PROBABILITIES.EDGE_PREFERENCE.RIGHT) {
      // Right edge
      x = Math.floor(Math.random() * edgeZoneX) + (maxX - edgeZoneX - marginX);
      y = Math.floor(Math.random() * rangeY) + marginY;
    } else if (edge < PROBABILITIES.EDGE_PREFERENCE.TOP) {
      // Top edge
      x = Math.floor(Math.random() * rangeX) + marginX;
      y = Math.floor(Math.random() * edgeZoneY) + marginY;
    } else {
      // Bottom edge
      x = Math.floor(Math.random() * rangeX) + marginX;
      y = Math.floor(Math.random() * edgeZoneY) + (maxY - edgeZoneY - marginY);
    }
  } else if (avoidCenter) {
    // Avoid center area
    ...
    // Standard random coordinates (range guarded above).
    x = Math.floor(Math.random() * rangeX) + marginX;
    y = Math.floor(Math.random() * rangeY) + marginY;
  }
