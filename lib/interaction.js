 ...
 * @param {number}  [options.waitMs=5000] - settle window per click (also the nav wait)
...
async function performTargetedClicks(page, selectors, options = {}) {
  const { realistic = false, waitMs = 5000, forceDebug = false } = options;
  const results = [];
  ...
  for (const raw of selectors) {
    ...
    let el = null;
    try { el = await page.$(selector); } catch (_) { el = null; }
    if (!el) {
      if (forceDebug) console.log(formatLogMessage('debug', `${INTERACTION_TAG} click_elements: no match for "${selector}" — skipping`));
