...
- `lib/` — 36 focused, single-purpose modules:
  ...
  - `eval-on-doc.js` — Fetch/XHR interception injected before page scripts (`evaluateOnNewDocument: true` per site, or `--eval-on-doc`). Three strategies: browser health check, full injection, then a minimal fetch-only fallback — but never after a CDP/Protocol error, which means the browser link is broken. Also exports `installReloadLoopGuard()`, which breaks a page that reloads itself: it counts main-frame navigations to the same URL (so it catches `reload`, `replace`, `href` and `<meta refresh>` alike) and disables script execution when the count passes `reload: N` plus slack. Driver-side because it has to be — `location.reload` is `[[Unforgeable]]`, and the in-page version this replaces had never once run. It disables script rather than aborting the navigation: aborting was measured leaving the page on `chrome-error://chromewebdata/`, which would hand grep/searchstring/screenshot an error page
