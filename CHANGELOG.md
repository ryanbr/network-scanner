...
- **`max_redirects` was documented wrongly in every respect.** README, `--help` and `nwss.1` all read "Maximum number of redirects to follow (default: 10; 0 = follow none)". Measured: it does **not** govern whether redirects are followed — Chrome follows them natively whatever the value, and at `0` the landed page is still loaded and its requests still captured. What it caps is how many redirects nwss **tracks**. And `lib/redirect.js` seeds `redirectChain` with the original URL, so its length is already 1 before any redirect is recorded and the guard `redirectChain.length >= maxRedirects` bails immediately for `0` **and** `1` — the two are identical — meaning a value of `N` tracks `N-1` redirects and the default of 10 tracks nine (confirmed across 0/1/2/3/10 against a cross-host 302). Tracking is what decides classification: a tracked destination joins the first-party set (unless `redirect_first_party: false`) and updates the final URL for output attribution, while an untracked one stays third-party and so remains eligible for `filterRegex`/`dig` capture. All three doc surfaces corrected, and the off-by-one plus the follows-regardless behaviour are now recorded at the guard itself. No behaviour change — the comparison is untouched.
