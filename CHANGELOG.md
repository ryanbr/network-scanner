...
### Changed
- **dig result cache cap raised 2000 → 4000 entries.** Headroom so a large multi-URL run doesn't evict still-hot domains mid-run (age-based eviction only kicks in above the cap). Entries are tiny (a domain string + short dig output), so the extra memory is negligible; the 20h TTL and disk persistence are unchanged.
