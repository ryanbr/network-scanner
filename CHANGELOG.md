...
### Changed
- **Fingerprint spoof bumped Chrome 148 → 150** (stable advanced; 148 reached EOL 2026-06-02). Updated `USER_AGENT_COLLECTIONS` (UA → `Chrome/150.0.0.0`), `CHROME_BUILD` → `7871.47` (current 150 stable), and the deterministic UA-CH GREASE — brand `Not/A)Brand` → `Not;A=Brand`, version `99` → `8`, and the brand-list **order** flips to `<grease>, Chromium, Google Chrome` (150 % 6 = 0). Grease version is now a `CHROME_GREASE_VERSION` constant (was hardcoded `99` in four spots) so HTTP `Sec-CH-UA*` (nwss.js) and JS `userAgentData` (fingerprint.js) stay single-sourced. The GREASE algorithm is documented in the fingerprint.js module header (verified against real Chrome 128/131/148) so future bumps are mechanical. Verified: JS brands/fullVersionList/UA self-consistent, HTTP↔JS agree, sannysoft 0 failed.
- **puppeteer 25.1.0 → 25.3.0** (bundled Chromium → Chrome 150). Lockfile-only bump; `package.json` range (`>=24.0.0`) unchanged. Stealth harness re-run after the update: sannysoft 30 passed / 1 warn / 0 failed, `webdriver: false`, UA + `chromeRuntimeVersion` consistently 148 — no spoof regression. (The spoof deliberately presents stable Chrome 148 regardless of the bundled 150 build.)
- **dig result cache enlarged: cap 2000 → 4000 entries, TTL 20h → 36h.** More headroom so a large multi-URL run doesn't evict still-hot domains mid-run (age-based eviction only kicks in above the cap), and a longer TTL so recurring domains survive longer across runs (now matching the whois cache's 36h). Entries are tiny (a domain string + short dig output), so the extra memory is negligible. Trade-off of the longer TTL: since A-records can change, a domain that moves off a matched IP range (e.g. leaves Cloudflare) can stay cached up to 36h; acceptable for the stable CF-fronted ad domains dig-gating typically targets. Only successful lookups are cached — transient failures still aren't, so a flaky drop never persists.

### Fixed
- **End-of-run dig-failure report** — a `warn` line now reports how many `dig` lookups exhausted every attempt (UDP failover + TCP fallback) without an answer: `N dig lookup(s) failed after UDP+TCP (flaky resolver) — dig-gated matches may have been missed`, followed by a `Failed dig:` line **naming the domains** (mirrors the `Fresh dig:` sample) so you can re-run/verify exactly the ones the flaky link cost — instead of guessing which of the "fresh" lookups failed. Previously a fully-failed dig was silent. Prints only when the count is non-zero (healthy runs stay quiet) and respects `--silent`. Failures aren't cached, so a re-run retries them; a persistent count is the cue for `--dig-retry-failed`, `--dig-max-concurrent` pacing, or a local caching resolver.
