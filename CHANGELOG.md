...
### Added
- **`scripts/test-stealth.js` now runs a self-consistency check** for whichever `--ua` is selected, before the third-party targets. The bot-detection pages are built to catch headless *Chrome*, so a non-Chrome UA scores their checks as "doesn't look like Chrome" and real contradictions hide in that noise; this instead asks whether the spoofed identity is coherent with itself — Chromium-only APIs under a non-Chrome UA, Firefox-only props present or missing for the wrong family, platform/vendor disagreeing with the UA, and a plugins/mimeTypes mismatch. Offline (`about:blank`), so it costs nothing and needs no third-party page. It is what surfaced both fixes above, and it currently reports four remaining leaks under `--ua=firefox`/`safari` (`performance.memory`, `navigator.deviceMemory`, `navigator.connection`, `navigator.scheduling`) which are now visible rather than silent.
