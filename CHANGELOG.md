...
### Fixed
- **End-of-run dig-failure count** — a `warn` line now reports how many `dig` lookups exhausted every attempt (UDP failover + TCP fallback) without an answer: `N dig lookup(s) failed after UDP+TCP (flaky resolver) — dig-gated matches may have been missed`. Previously a fully-failed dig was silent, so a flaky resolver could quietly drop dig-gated matches with no visible signal. Prints only when the count is non-zero (healthy runs stay quiet) and respects `--silent`. Failures aren't cached, so a re-run retries them; a persistent non-zero count is the cue to add a local caching resolver or lean on `--dig-max-concurrent` pacing.
