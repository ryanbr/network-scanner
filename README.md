...
| `cookies`            | Object or Array | – | Cookies set **before** the page loads — for sites that gate on a cookie during the initial document load. Short form `{"consent": "granted"}` scopes to the site's own host at path `/` — **host-only**, so it is *not* sent to a subdomain: a site that redirects `domain.com`→`www.domain.com` needs the long form with `"domain": ".domain.com"` (the leading dot is what makes Chrome store it domain-scoped, and unlike `local_storage`/`session_storage`, which match the whole registrable domain, the cookie is otherwise dropped on that redirect); long form `[{"name":…,"value":…,"domain":…,"path":…,"secure":…,"httpOnly":…,"sameSite":"Strict\|Lax\|None","expires":…}]` gives full control. `expires` is **seconds** since the epoch (`Math.floor(Date.now()/1000)`) — a past value makes the browser discard the cookie and a millisecond value gets clamped, so both are warned about, as are unrecognised per-cookie keys. Non-string values are stringified. Applied after `clear_sitedata` and re-seeded before each reload, so the cookie is present on every load. **Scoped to the entry**: seeded cookies are removed when the URL finishes, so a later site on the same host does not silently inherit them |
| `local_storage`      | Object or Array | – | Entries written to `localStorage` **before** the page loads, for gates that read Web Storage instead of a cookie. Short form `{"rta_terms_accepted": true}`; long form `[{"name":…,"value":…}]`. Numbers/booleans are stringified, objects/arrays are `JSON.stringify`d. Installed via `evaluateOnNewDocument`, so it runs before the page's own scripts on every document — reloads and `clear_sitedata` need no re-seed. Written only in the top document, never in a cross-origin frame, and matched on the registrable domain (via `psl`) rather than the origin, so an `http`→`https` upgrade, a port change or a redirect to another subdomain is still seeded — wider than a seeded cookie, which is host-only unless given an explicit `.domain`. A redirect to a different registrable domain is refused. **Scoped to the entry**: removed when the URL finishes, across every origin the site was seen on (reference-counted, so concurrent URLs of the same site are safe) |
