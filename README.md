...
| `local_storage`      | Object or Array | – | Entries written to `localStorage` **before** the page loads, for gates that read Web Storage instead of a cookie. Short form `{"rta_terms_accepted": true}`; long form `[{"name":…,"value":…}]`. Numbers/booleans are stringified, objects/arrays are `JSON.stringify`d. Installed via `evaluateOnNewDocument`, so it runs before the page's own scripts on every document — reloads and `clear_sitedata` need no re-seed. Written only in the top document, never in a cross-origin frame, and matched on the registrable domain (via `psl`) rather than the origin, so an `http`→`https` upgrade, a port change or a redirect to another subdomain is still seeded — the same breadth a `.domain.com` cookie has. A redirect to a different registrable domain is refused. **Scoped to the entry**: removed when the URL finishes, across every origin the site was seen on (reference-counted, so concurrent URLs of the same site are safe) |
