...
| `click_elements`     | String[] | - | After load, click these CSS selectors **in order** — reach content via organic navigation/gesture instead of a direct load (e.g. `["a[href*='/movie/']", ".play"]` to click a link then a play button). The request interceptor stays attached, so the post-click page's requests are matched against `filterRegex`/`dig` as usual. A click that navigates is followed; later selectors query the resulting page. Honors `realistic_click`; missing elements are skipped (never fails the scan) |
| `click_wait`         | Integer | `5000` | Settle/navigation wait (ms) after each `click_elements` click; capped at half the per-URL timeout |
