...
| `--dig-max-concurrent <number>` | Cap concurrent `dig` subprocesses (default 6, 0 = uncapped). Under a high `--max-concurrent` the scanner can otherwise fire that many simultaneous lookups at the same resolvers, provoking rate-limit drops (rough on WSL2's UDP-through-NAT path). Only genuine cache-miss lookups contend for a slot; cache hits and deduplicated lookups bypass it. Also `dig_max_concurrent` in `.nwssconfig` |
| `--dig-retry-failed [number]` | On total `dig` failure (UDP failover + TCP fallback both empty), make N extra TCP attempts with a ~3s backoff before giving up (bare flag or `dig_retry_failed: true` = 2; 0 disables; capped at 5). For bursty/flaky resolvers where a fresh attempt seconds later succeeds; the recovered result is captured **this run** via the normal match path. Off by default; extra latency applies to already-failing lookups only. Also `dig_retry_failed` in `.nwssconfig` |
...
| `blocked`            | String or Array | - | Domains or regexes to block during scanning. A bare string is treated as one pattern; an empty string means none |
...
| `js_redirect_timeout` | Milliseconds | `5000` | Time to wait for JavaScript redirects. The effective spend is a **third** of this value, on every URL whether it redirects or not (the poll stops at the first quiet check). `0` skips the wait entirely — the cheapest setting. What it gives up is narrower than it sounds: only redirects that land *after* navigation settles stop being tracked, while HTTP 30x and inline `location.href` redirects commit during load and are still caught. The landed page's requests are captured either way |
...
| `cookies`            | Object or Array | – | Cookies set **before** the page loads — for sites that gate on a cookie during the initial document load. Short form `{"consent": "granted"}` scopes each cookie to the whole **site** at path `/` — `.domain.com`, the registrable domain of the URL resolved with `psl`, so it survives an apex↔`www` redirect and matches the breadth `local_storage`/`session_storage` use. The leading dot is what makes Chrome store a cookie domain-scoped; a `"domain"` written *without* one is host-only (exact host, never a subdomain), which is how to ask for host-only on purpose — nwss warns when such a host-only domain is a *parent* of the URL's host, since it could then never be sent at all. A host with no wider scope (an IP, `localhost`) stays host-only. Long form `[{"name":…,"value":…,"domain":…,"path":…,"secure":…,"httpOnly":…,"sameSite":"Strict\|Lax\|None","expires":…}]` gives full control. `path` must be absolute — a relative one is rejected by the browser, so nwss warns and uses `/` (the only path a load-time gate is read at). `expires` is **seconds** since the epoch (`Math.floor(Date.now()/1000)`) — a past value makes the browser discard the cookie and a millisecond value gets clamped, so both are warned about, as are unrecognised per-cookie keys. Non-string values are stringified. Applied after `clear_sitedata` and re-seeded before each reload, so the cookie is present on every load. One cookie the browser refuses (say an unusable `domain`) costs only that cookie — the rest are retried individually and the rejected one is named. **Scoped to the entry**: seeded cookies are removed when the URL finishes, so a later site on the same host does not silently inherit them |
| `local_storage`      | Object or Array | – | Entries written to `localStorage` **before** the page loads, for gates that read Web Storage instead of a cookie. Short form `{"rta_terms_accepted": true}`; long form `[{"name":…,"value":…}]`. Numbers/booleans are stringified, objects/arrays are `JSON.stringify`d. Installed via `evaluateOnNewDocument`, so it runs before the page's own scripts on every document — reloads and `clear_sitedata` need no re-seed. Written only in the top document, never in a cross-origin frame, and matched on the registrable domain (via `psl`) rather than the origin, so an `http`→`https` upgrade, a port change or a redirect to another subdomain is still seeded — the same breadth a seeded `cookies` entry has (both resolve it through `lib/site-scope.js`). A redirect to a different registrable domain is refused. **Scoped to the entry**: removed when the URL finishes, across every origin the site was seen on (reference-counted, so concurrent URLs of the same site are safe). Removal needs a live page on the origin — if the seeding page has already closed, a concurrent URL of the same site is used instead, and when nothing is left on that origin the entries survive the entry and a warning names it |
| `session_storage`    | Object or Array | – | Same as `local_storage` but writes `sessionStorage`. Needs no teardown — `sessionStorage` is scoped to the page, which closes when the URL finishes |
...
| `capture_popups_signal` | Boolean | `false` | Count a `$popup` pattern from the `--block-ads` lists as a match, so a popup landing on a known popunder endpoint is captured even when `filterRegex` misses it. Neither engine can *block* a `$popup` rule, but most such patterns appear nowhere else in a list, so as a set they catalogue known popunder endpoints. Promoted hits take the same path a `filterRegex` match does (`ignoreDomains`, party gates and `whois`/`dig` validation still apply — so on a site that configures `whois`/`dig`, a hit triggers those lookups for the popup's domain and is only captured if they pass). Requires `capture_popups` + `--block-ads`. Off, hits are still tallied and reported at scan end |
...
| `js_redirect_timeout` | Milliseconds | – | Default JS-redirect wait for any site that doesn't set its own; per-site values win. That wait costs roughly a **third** of its value on every URL whether or not the URL redirects, so this exists to avoid repeating the same line for every site in a large config. `0` disables it — see the per-site entry for what that gives up |
...
```bash
node scripts/test-seeding.js                  # everything (~1 min, mostly the scans)
...
node scripts/test-seeding.js --verbose        # per-check detail
