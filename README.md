...
| `local_storage`      | Object or Array | – | Entries written to `localStorage` **before** the page loads, for gates that read Web Storage instead of a cookie. Short form `{"rta_terms_accepted": true}`; long form `[{"name":…,"value":…}]`. Numbers/booleans are stringified, objects/arrays are `JSON.stringify`d. Installed via `evaluateOnNewDocument`, so it runs before the page's own scripts on every document — reloads and `clear_sitedata` need no re-seed. Written only in the top document, never in a cross-origin frame, and matched on hostname rather than origin so an `http`→`https` redirect is still seeded. **Scoped to the entry**: removed when the URL finishes (reference-counted, so concurrent same-origin URLs are safe) |
