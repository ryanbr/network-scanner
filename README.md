...
| `capture_popups_signal` | Boolean | `false` | Count a `$popup` pattern from the `--block-ads` lists as a match, so a popup landing on a known popunder endpoint is captured even when `filterRegex` misses it. Neither engine can *block* a `$popup` rule, but most such patterns appear nowhere else in a list, so as a set they catalogue known popunder endpoints. Promoted hits take the same path a `filterRegex` match does (`ignoreDomains`, party gates and `whois`/`dig` validation still apply). Requires `capture_popups` + `--block-ads`. Off, hits are still tallied and reported at scan end |
